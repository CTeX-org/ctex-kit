...
- **前置预防**：曾有 `scripts/sync-l3backend.sh` 在 `l3build doc` 之前补齐匹配版本的 backend，从源头消除这个已知成因；该脚本已随上游把 l3backend 并入 l3kernel 而在 #1074 撤除。**注意这使本缺口更加裸露**：那条防御针对的是一个已知成因，而本缺口是「任何原因导致的正文污染都检不出」，下一个同类上游问题不会再有前置预防兜着。
