...
## Follow-up
- 最终落地方案并不是全局重定义 `\char`，而是把这次经验固化为更低侵入的兼容策略：
  1. 承认 XeTeX interchar 在 token 层面无法自动分辨 `\char` 输出与直接字符输入，因此 #407 不存在“完全不碰 `\char` 仍自动修好所有场景”的实现路线。
  2. 为显式需要绕过 interchar 的场景新增 `\xeCJKchar`，供用户与第三方宏包直接调用。
  3. 对已知真实受害者 `mtpro2` 直接做定点补丁，在 `\overcbrace` / `\undercbrace` 入口处关闭 interchar。
  4. 对其他包则保留文档化的手动 patch 指南：直接把局部 `\char` 改成 `\xeCJKchar`，或在命令外层用分组 + `\makexeCJKinactive` 包装。
- 因此，后续再遇到类似 xeCJK 字体误切换问题时，检查顺序应更新为：先确认触发对象是否为 `\char` 原语；若是，不要默认考虑全局改写原语，而应优先判断是否适合落到“新显式命令 / 已知包定点补丁 / 用户手动包装”三层策略中的哪一层，并继续用真实场景对应的盒子结构编写回归测试。
