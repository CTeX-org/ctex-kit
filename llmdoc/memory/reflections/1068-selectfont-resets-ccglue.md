...
2. **`\ccwd` 会随用户的 `kanjiskip` 变化，这是既有语义不是本次副作用。** 修好后
   ...
   **但这条对照只够证明「内部自洽」，不够证明「该行为正确」** —— 用户看到 PR 截图里
   AFTER 第二行多出缩进时问「这正常吗」，我才发现自己只有前者。补两条真正的依据：

   - **手册的定义**（`ctex.dtx` 对 `\ccwd` 那节）：「汉字字宽是相邻两个汉字中心之间的
     距离，**包含字距在内**。因此修改字距会间接修改字宽。」`autoindent` 默认取 `2\ccwd`，
     所以字间距变大时缩进跟着变宽是文档承诺的语义。`\ziju` 一节也重申「这个命令会影响
     `\ccwd` 的值」。
   - **跨引擎跨版本对照**：XeTeX 在**未修改的 v2.6.4** 上，同一份样例的 `\parindent`
     就已经是 41.07874pt、`\ccwd` 20.53937pt，与本次修好后 LuaTeX 的读数一致。LuaTeX
     先前停在 21.07874pt，正是因为间距被错误重置、`\ccwd` 也就没变。

   教训：「与既有配置一致」和「符合设计意图」是两个命题。前者可以靠内部对照证明，
   后者必须回到文档或规范的原文。整个设计若有问题，它照样内部自洽 —— 只用前者回答
   「这正常吗」是答不上的。
