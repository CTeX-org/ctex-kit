...
description: 记录 #1046（l3doc \meta 左右 CJKecglue 不对称）与 #1047（hyperref 锚点前的异常间距）的机制事实与两类归因错误：「注册点的字体上下文是语义的一部分」，以及同一份文档里三次把观察到的现象推成未经验证的机制（先误判分派依据、再误判出口穷尽性、最后为包装失败编了一个隔离实验一测就倒的成因并据此放弃覆盖），均由盲审推翻；另含把既有测试全绿当成缺陷不存在证据、低位 box 寄存器被被测命令占用、本地 TeX Live 中途漂移等验证教训
metadata:
  type: feedback
---

# 反思：#1046 / #1047 注册点的字体上下文，与四次被推翻的锚点机制陈述
...
`{`、`[`、`(` 实字符，本身就构成正常的 CJK→Default 边界，去掉内层 capture 前后宽度与可见
排版结果完全相同，节点列表上少了一对零效果的 `default` marker kern），但仍留在保留表里，防止用户接口把通用 hook 叠到共享的内部实现上。

### #1047：锚点有多个出口，按调用点区分

hyperref 的行内锚点有多个出口。**已覆盖**三处，各注册一次 `transparent`：
...
3. `\__hyp_target_raise:n`（`hyperref.sty:6458`）：`\phantomsection` 与 `\MakeLinkTarget`
...
**然后我第三次犯了同一类错误。** 我试着补注册第三个出口，两条路看起来都不通：
`\__hyp_target_raise:n` 是 expl3 私有函数，LaTeX hook 机制明确拒绝
（`Generic hooks cannot be added to ...`）；改用现成的
`\@@_boundary_wrap_transparent_onearg:NN` 后，实测把 `\spacefactor` 赋值写进了 `pdf:dest`
名字、把锚点名 `section*.1` 排成可见文本，盒宽由 41.66002pt 变成 84.49002pt。我把这个故障
归因为「begin 钩子里的 `\Hy@SaveSpaceFactor` 赋值被卷进了参数展开」，据此判定「需要单独
设计适配器」，回退并把缺口写成已接受限制（TEST 7 断言「缺口仍然存在」）。

第三轮盲审用**隔离实验**推翻了这个成因，三个独立证据：

1. `\@@_boundary_hmode_transparent_begin:` 的函数体里**根本没有** `\spacefactor` 赋值——
   只有 `\int_compare:nNnTF`、`\seq_gpush:Nn`、`\@@_boundary_capture_begin:` 与
   `\tl_gset:cn`。被写进 `pdf:dest` 名字的那个赋值来自 hyperref 自己的
   `\Hy@SaveSpaceFactor`。
2. 保留同一对 begin/end、只把参数写成 `#1 {##1}` 而非 `#1 ##1`，故障完全消失（41.66002pt）。
3. 反过来，**完全不挂任何 xeCJK 钩子**、仅做无花括号透传，同样复现故障（81.16002pt）。

真正的原因是参数转发丢了花括号：`\__hyp_target_raise:n` 会把参数再次用作 `\hbox:n` 的
内容，分组消失后紧随其后的 `\Hy@SaveSpaceFactor` 被吸进 `\hyper@anchorstart` 的参数。
于是新增 `\@@_boundary_wrap_transparent_onearg_braced:NN`（与既有变体唯一区别就是那对
花括号），第三个出口正常覆盖，`\phantomsection` 回到 41.66002pt。原先断言「缺口仍在」的
那一项改成了正向断言（最终编号为 TEST 8、TEST 9）。

**这一次错得最重**：前两次是把事实之间的联系说错，这一次是为一个真实故障编了一个成因，
并据此放弃了一个本可当场关闭的缺口——错误的机制解释直接改变了技术决策，而不只是文档措辞。

**然后是第四次。** 覆盖第三个出口之后，我写下「三个出口全部注册」，并把这个数字写进 dtx
实现注释、CHANGELOG、架构文档、决策文档、`index.md` 与测试注释。第四轮全范围复核用同一支
计数器探针推翻了它：`\pdfbookmark` 直接写 `\hyper@anchorstart{...}\hyper@anchorend`，四个
候选函数里只有 `\hyper@anchorstart` 计数为 1，`\pdfbookmark` 右侧仍缺 3.33pt
（38.33002pt 对 oracle 41.66002pt）。同类裸调用在 hyperref 与各驱动里还有若干处。

我试了两种就手的补法，都不行：注册 `\@pdfm@dest`（`\hyper@anchor` 与 `\hyper@anchorstart`
的共同下游）让盒宽涨到 166.49002pt 并报出十余处错误，因为它的参数含待展开内容；注册
`\hyper@anchorstart` 本身不报错，但也不生效——`\pdfbookmark` 仍 38.33002pt，而已覆盖的
三处不受影响。为什么 transparent 在这个入口上无效尚未查明。

于是按「已知未覆盖」处理：TEST 10 断言缺口仍在，另立议题跟踪。**更重要的是改掉写法本身**
——文档从此只维护「已覆盖」与「已知未覆盖」两份清单，不再给出出口总数。理由很直接：这个
总数连错四次，而清单里的每一条都能被单条探针核查，总数不能。要给上游包的内部出口计数，
需要的是穷举式审计（grep 全部 `\hyper@anchorstart`／`\@pdfm@dest` 调用点并逐一实测），
不是从「修好了报告的现象」倒推。

**教训**：「A 缺了会坏、B 缺了也会坏」只证明「A 和 B 都在路径上」。它既不证明「按某个条件
在 A 与 B 之间分派」，也不证明「只有 A 和 B」；而观察到一个故障也不证明你对成因的解释。
这些是彼此独立的命题：

- 分派依据是关于控制流的命题，探针是给候选函数各加计数器；
- 穷尽性是关于「还有没有下一个」的命题，需要能说明用什么手段排除了它——我两次都没有做
  这一步，只是从「报告的现象消失了」倒推；
- 成因是关于「去掉这个因素故障还在不在」的命题，探针是隔离实验。我把包装失败归因给
  xeCJK 的钩子，而只要「不挂钩子也复现」一条就能推翻——一次编译的成本。
...
## 元教训：我批评了自动分析的归因错误，然后连着犯了四次同类错误
...
3. 承认第三个出口之后，我又为「它套不上现成包装」编了一个成因（begin 钩子的赋值被卷进
   参数展开），并据此放弃覆盖、把缺口写成已接受限制。第三轮盲审的隔离实验推翻了它：那个
   钩子体内没有该赋值，不挂钩子也复现，改成花括号转发即修复。**这次错误改变了技术决策**，
   不只是措辞。

五次错误的结构完全一致：**手里有一个真实现象，为它编了一个听起来合理的解释，没有为那个
解释单独设计探针。**
...
| 我，第三次 | 套现成包装后锚点名被排出 | 所以是 begin 钩子的赋值被卷进参数，需要新适配器 |
| 我，第四次 | 覆盖三处后 issue 的现象全消失 | 所以出口只有三个 |
...
- **联系、穷尽性、成因都是独立于现象的命题。** 「A 缺了会坏、B 缺了也会坏」既不证明「按某
  条件在 A 与 B 之间分派」，也不证明「只有 A 和 B」；观察到一个故障也不证明你对成因的解释。
  控制流用计数器，穷尽性要说明如何排除下一种，成因用隔离实验（去掉你认定的因素，看故障
  是否仍在）。写「全部」「只有」「因为 X 所以坏」之前先问「我实测过吗」，尤其在准备把它
  提升为跨任务判据、或据它放弃一项修复时。
- **某类断言反复出错时，换掉断言的形式，而不只是修正它的内容。** 前三次我都是「改对这一句」
  然后继续写下一个同类断言，于是错了第四次。第四次改的是写法：不写总数，只维护「已覆盖」
  与「已知未覆盖」两份清单。清单的每一条对应一次可重跑的探针，总数不对应任何单次实验——
  它只能由穷举审计支撑，而我从来没做过那个审计。**当同一形态的错误出现第三次，要问的不是
  「这次怎么写对」，而是「这种句式我有没有能力保证它对」。**
- **「实测过」要说清实测的是什么，并检查探针本身够不够用。** 我写过「去掉内层 capture 前后
  节点列表完全相同（实测）」，而当时做的是宽度比对。补做 `\showbox` 比对时我又用了不足的
  探针（单入口 + 含 `\meta` 的预热行），得到「无差异」并把它写进五处；换成同一 `\hbox` 里
  放两个入口（`\Arg` 加 `\oarg`）就看到 base 每入口多一对 `default` marker kern。宽度确实
  不变，但断言得改成「宽度与可见排版结果相同；节点列表少一对零效果 marker kern」。标注
  「实测」反而让错误结论更容易被当成已验证。
...
- recorder 同步 `reference/build-and-test.md`：登记
  ...
  `hyperref-anchor-ecglue01.lvt/.tlg`（12 项断言），xeCJK 标准回归总数从 120 增至 122；
