...
# 反思：#1046 / #1047 注册点的字体上下文，与三次被推翻的锚点机制陈述
...
hyperref 的行内锚点有三个出口，各注册一次 `transparent`：
...
### 三次把观察到的现象推成未经验证的机制（本次第二至第四个归因错误）
...
花括号），第三个出口正常覆盖，`\phantomsection` 回到 41.66002pt。TEST 7／TEST 8 也从
「断言缺口仍在」改成了正向断言，该测试现为 11 项。
...
## 元教训：我批评了自动分析的归因错误，然后连着犯了三次同类错误
...
四次错误的结构完全一致：**手里有一个真实现象，为它编了一个听起来合理的解释，没有为那个
解释单独设计探针。**
...
- recorder 同步 `reference/build-and-test.md`：登记
  ...
  `hyperref-anchor-ecglue01.lvt/.tlg`（11 项断言），xeCJK 标准回归总数从 120 增至 122；
