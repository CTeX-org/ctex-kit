...
hyperref 的行内锚点有两个出口，各需一次 `transparent` 注册：
...
2. `\Hy@raisedlink`（`hyperref.sty:2104`）：承接需要抬升的锚点——目录条目、脚注——
   以及下游手工包裹的写法，例如 ctxdoc 的 `\exptarget`（`ctxdoc.cls:193`）定义为
   `\Hy@raisedlink{\hypertarget{expstar}{}}`。它排出 `\penalty\@M` 加 `\smash` 后的
   `hbox(0+0)x0`，用来盛放 `pdf:dest` special。

**这一节最初写错了**：写成「非空目标的 `\hypertarget` 走 `\Hy@raisedlink`、空目标走
`\hyper@anchor`」。详见下文「判别力互不重叠只证明有两个出口，不证明按什么分派」。
...
### 判别力互不重叠只证明有两个出口，不证明按什么分派（本次第二个归因错误）
...
**教训**：「A 缺了会坏、B 缺了也会坏」证明的是「A 和 B 都在路径上」，不是「按某个条件在
A 与 B 之间分派」。后者是关于控制流的独立命题，需要独立探针——这里就是给两个命令各加一个
计数器，一次运行就能定论。这与 #1043 确立的「事实、原因、后果是三个独立命题，各自都要单独
验证」是同一条规则：我验证了「两个注册都必要」这个事实，却给它编了一个未经验证的机制解释。

判据仍然是**读分派函数的分支并实测**（这里是 `\hyper@@anchor` 与 `\@hyper@@anchor`），
而不是看公开命令的参数形态。这与 #1037「根因是代码事实，写成可 grep 的模式穷举全部出现
位置」同源，但换了个维度：那次穷举的是「同一件事在代码里做了几遍」，这次要确定的是「一个
入口按什么条件分派到哪个出口」。
...
## 元教训：我批评了自动分析的归因错误，然后犯了同一类错误

本次反思一开始就把「自动 issue 分析把可 grep 的代码事实归因成视觉错觉」列为最严重的判断
错误，还写下「否定性结论要说明搜索了什么模式、为什么能穷尽」这条规则。然后我在同一份文档
里写下了「非空目标的 `\hypertarget` 走 `\Hy@raisedlink`、空目标走 `\hyper@anchor`」——同样
是一个未经实测的机制解释，同样被写进了架构文档和跨任务规则，同样由盲审用一次计数器探针
推翻。

两次错误的结构完全一致：**手里有一个真实现象，为它编了一个听起来合理的机制解释，没有为
那个解释单独设计探针。** 自动分析那次的现象是「既有测试全绿」，编的解释是「所以是视觉
差异」；我这次的现象是「两个注册都必要」，编的解释是「所以按目标是否为空分派」。

可推广的部分：

- 修复奏效、测试判别力互不重叠，都只是**现象**。把现象叙述成机制时，那句机制陈述是一个
  需要独立证据的新命题。#1043 已经确立「事实、原因、后果是三个独立命题」，本次说明它还有
  一层——**「若干个独立事实」与「把它们联系起来的机制」也是相互独立的命题**。
- 探针成本往往极低。这次只需给两个命令各包一个 `\newcount` 计数器，一次编译就能定论；成本
  远低于把错误陈述写进四份文档、再由盲审发现并逐处更正。**写下「A 走这条、B 走那条」之前，
  先问「我实测过吗」。**
- 我在四份文档里重复了这个陈述，其中两处（`architecture` 与 `lessons-learned`）还把它提升
  成了跨任务判据。**重复陈述不增加可信度，只增加错误的传播面**；提升为通用规则前，应当对
  该规则本身再要一次证据。
- 盲审的价值正在这里：它不接受我的叙述，直接去读 `\@hyper@@anchor` 的分支并动手实测。这是
  「独立审查须在无父上下文的新会话中进行」这条流程规则的一次具体回报。
...
- **同一类节点可能有多个出口，判据是读分派函数的分支并用计数器实测。** hyperref 的行内
  锚点有 `\hyper@anchor`（承接全部 `\hypertarget`）与 `\Hy@raisedlink`（承接目录、脚注
  与下游手工包裹）两个出口。「A 缺了会坏、B 缺了也会坏」只证明两者都在路径上，不能推出
  按什么条件分派——后者要单独探针。
...
- **若干个独立事实与把它们联系起来的机制，是相互独立的命题。** 「A 缺了会坏、B 缺了也会
  坏」证明两者都在路径上，不证明「按某条件在 A 与 B 之间分派」；后者需要针对控制流的独立
  探针（例如给两个候选函数各加计数器）。写下机制陈述前先问「我实测过吗」，尤其在准备把它
  提升为跨任务判据时。
...
- recorder 同步 `reference/build-and-test.md`：登记
  ...
  `hyperref-anchor-ecglue01.lvt/.tlg`（8 项断言），xeCJK 标准回归总数从 120 增至 122；
