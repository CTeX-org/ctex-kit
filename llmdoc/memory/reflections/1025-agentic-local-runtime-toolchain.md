...
llmdoc publisher 先推送候选分支，再调用 GitHub API 创建 PR。这两次外部写入不是一个原子操作：
分支推送成功以后，网络或 API 故障仍可能使建 PR 失败。旧实现把“远端分支存在但没有 open PR”一概
视为未知占用；同一份已经校验的候选重跑时也会在建 PR 以前被拒绝，只能由维护者手工清理分支。

修正后，publisher 以经过 artifact 清单校验的 `candidate_sha` 区分三种状态：远端 head 与当前候选
完全相同时，允许重跑并补建 PR；旧 head 已经进入本轮 `base_sha` 时，说明上一轮 PR 已合并但分支
没有删除，允许用精确 lease 更新；远端 head 既不同、又没有合入 base，而且没有带认证 marker 的
