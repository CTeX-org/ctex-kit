...
真正的原因是：**LaTeX 环境默认把 `&` 设为 4，而 `\ExplSyntaxOn` 不去动它**。
这个区别不是文字游戏——原说法暗示「expl3 régime 保证了 4」，实际是「没有任何机制保证 4，
它随调用方环境漂移」。后者恰恰让 `\c_@@_alignment_tl` 这个防御**更有必要**。
盲审还用变异证明了这一点：把环境 `&` 设为 12 且实现改回字面 `{ & }` 后，门禁 EXIT=1。

**教训（比第 1 条更精确）**：第 1 条的错误是「读数是废的」，这一条的错误是
**「读数正确，但我为它编了一个没验证的原因」**。修正一条假陈述时，替换进去的新解释
...
两轮盲审各报 1 blocking / 2 important / 2 minor，**十个 finding 全部由盲审发现，我的自检
一个都没抓到**。而且两轮的 blocking 是**同一个失效模式的两次发作**：
...
「构造 X 不成立的场景，Y 还成立吗？」——第二轮的 `\cctab_select:N` 实验正是这种检验。
