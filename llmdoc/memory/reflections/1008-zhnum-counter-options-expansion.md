...
这与本文件前面 What Went Wrong 一节里已经记的「死路 1、2」（观察不可展开命令的行为，
只有「让它执行」一条路）是同一根源，但这次的教训更严重一层：**我在已经写下那条教训
之后，仍然在同一份改动里留下了一个犯同样错误的断言**。写下一条教训不等于已经把它应用
到手头所有代码上——补完教训后要回头扫一遍同一批产物里是否还有同型问题。
...
独立命题**，同步性校验对确定性缺陷零判别力。这与 lessons-learned 里讲
「跑了但什么也没校验」的 job 比没有 job 更危险的那一条相邻但不同。
...
1. **观察不可展开命令的行为，只有「让它执行」一条路**：`\typeout`/`\tl_set:Nx`/
   ...
   完全没有判别力。这条与 #1043 反思里「探针静默先自证有效」同属一类——先确认观察手段本身
   有没有能力看到你要断言的东西。
...
4. **盒子度量选哪一维要先验证它真的会变**：缺字时宽度可能 collapse 到同一个值，高度
   ...
   确认它们不同」。
5. **手册措辞会塑造用户对「是不是设计限制」的判断**：本次报告者正是被「不可展开」这句话
...
9. **对 protected 命令（`\NewDocumentCommand` 定义的宏）用 `\tl_set:Nx`／`\tl_set:Nn`
   一类捕获结果的断言恒真**——只能拿到命令名本身，这与本文件已有的「观察不可展开命令的
   行为，只有「让它执行」一条路」是同一条规则；本次的额外教训是**这条规律会在同一批
   改动里复发**：写下一条教训后仍需要回头检查同一批产物是否还有同型问题，不能假设
   写过一次就不会再犯。
...
10. **测试输入的取值本身要能区分被测的各条路径**：`legacy-entry01` 的字形断言一开始只
    ...
    读法分开。这与「聚合度量选哪一维要先验证它真的会随被测行为变化」是同一类问题的输入侧版本。
11. **引用统计数字前先确认量的就是所声称的那件事**：我曾用「汉字紧接西文／反引号的
    出现数重排前后一致（189 → 189）」来证明没有丢 CJK／西文之间的空格。拆开那 189 处
    后，**其中拉丁字母 0 处**——那个字符类一次也没匹配到我声称在数的东西，数出来的全是
    Markdown 标记和标点；该计数在重排后也并非不变。按对应缺陷的口径（汉字紧接拉丁
    字母）重新核对，结论成立，但那句统计与结论毫无关系。
    附带一条过程教训：我起初只承认「口径不准」，此后连续两轮盲审都在这段统计叙述里
    找出新的不准确处（分解加不起来、单文件与多文件范围混用、「始终为 0」过强）。
    补救办法不是再补一版更细的数字，而是把它压缩成一条原则加一个可复现的关键事实——
    叙述里每多一个精确数字，就多一处需要维护、也多一处会出错的地方。
12. **「生成物与源同步」和「生成物本身正确」是两个独立命题**：`check-changelog.yml` 的
    「重新生成 + git diff」新鲜度校验只能证明前者，对确定性缺陷（生成脚本本身有 bug，
    两次生成结果一致但都错）零判别力，需要单独一道校验源内容本身是否正确。
13. **改一处引用后要用脚本枚举同类引用再逐个核对，不能靠记忆定位**：本次「引用其他小节
    要复制标题原文」连栽四轮，每轮都只改了被点名的那一处，直到改用脚本枚举本分支新增行
    里的**所有**「……」引用并逐个与真实标题比对才一次性止住。这与已有的「先用 `grep -rn`
    列全部出现位置」同属一类，但这次的失败模式是「每次只改一处」而不是「漏改某处」。
14. **叙述里每多一个精确数字，就多一处需要维护、也多一处会出错的地方**：统计口径类描述
    连栽三轮，每轮都补一版更细的数字，真正止住它的是把整段压缩成一条原则加一个可复现的
    关键事实，而不是继续加细节。适用于任何「为增强可信度而堆数字」的表述场景。
15. **往既有文档追加内容前先看现有条目的形态（行宽、折行与否、字段结构）并照着写**：
    `lessons-learned.md` 六十余条规则全是单行不折行，新加的两条一旦折行就成为全文档
    唯一的异类，此前反复纠缠的碎行、并回、列宽争议全部由这一处格式偏离派生；按既有格式
    改回单行后，整类问题一次性消失。
...
PR **#1058**，目标分支 **`zhnumber/maintaining`**（不是 master）；CI 非 skip 的 38 项
全部 SUCCESS；bot review（Codex 主链路）**APPROVE，0/0/0**。复现资产在 `gh-assets` 分支
`issues/1008/`（MWE、修复前后两张对照图、两侧 `.toc` 实际内容），commit `dabf1a94`。

独立审计（close-local-code-review）跑了 **21 轮盲审**（1 首轮 + 18 增量 + 2 最终全范围），
每轮在固定 commit 的离线快照克隆里进行（`bwrap --unshare-net` 禁网）。累计 **76 条
finding（7 阻塞 / 27 重要 / 42 小）**，全部 fixed；最终全范围轮 0/0/0 APPROVE；终审收据
判 PASS（见 `.code-review/closure-receipt-1008-final2-597f114d.md`）。

### 审计过程本身的教训

21 轮里只有 **5 条**是代码/测试层面的真问题（度量不足以固定字形、TEST 4 只覆盖一半
入口、一位数下两条路径输出恒等、两条守卫各自的判别力、`\c_false_bool` 的跨编译隐患），
其余 **71 条是文档表述反复收敛**。两个最刺眼的复发模式：

- **「引用其他小节要复制标题原文」连栽四轮**：先按相对位置指代（「上一条」而该条在下方
  88 行）、再按正文措辞指代、再漏掉标题内层的引号、再把「是不是设计限制」写成
  「这是不是限制」。每轮我都只改被点名的那一处。
- **「统计口径要对应所声称的性质」连栽三轮**：先是口径写错（字符类把全角标点算作西文）、
  再是分解加不起来（6+104≠189）、再是单文件与多文件范围混用 + 「始终为 0」过强。每轮我
  都补一版更细的数字。

**共同根因**：我每轮都在「补一版更细的细节」，而不是消除产生错误的结构。真正止住它们
的两个动作是：

1. 标题引用那条——改用脚本枚举本分支新增行里**所有**「……」引用，逐个与真实标题比对
   （而不是凭记忆 grep 单个词组）；
2. 统计口径那条——把整段压缩成「一条原则 + 一个可复现的关键事实」，删掉十一个精确
   数字断言。**叙述里每多一个精确数字，就多一处需要维护、也多一处会出错的地方。**

还有一条相关观察：`lessons-learned.md` 六十余条规则全是 `**Rule**:` / `**Why**:` 各占
一行不折行，而我新加的两条是全文唯一的折行段落；此前反复纠缠的碎行、并回、列宽区间之
争，全是这个**格式偏离**派生出来的。按既有格式取消折行后，整类问题一次消掉。一般化：
**往一份既有文档里加内容前，先看它现有条目的形态（行宽、折行与否、字段结构），照着写；
偏离既有形态会派生出一串与内容无关的问题。**

### evidence_completeness 缺陷

终审收据判 PASS，但记下一条 `evidence_completeness` 失败：21 份 run manifest 里
`events[].at` 的 129 个时间戳是我**事后按整分钟手写补记**的（103 个秒值为 `00`、20 个
为 `30`），不是事件发生时采集的。后果是 19/21 个 run 的 `blind_report_archived` 时刻
早于报告文件 mtime（最大偏离 35 分钟，物理上不可能）。

**不影响结论**：关键的 freeze-before-history 次序不依赖这些时间戳，终审用文件 mtime
独立重建并证实；同期性最强的一环（每轮 staged patch 与 commit patch 摘要相等，且
21/21 与真实 `git diff <commit>^ <commit>` 逐字节命中）也与时间戳无关。

**可操作改法**：每次追加事件用 `date -Is` 当场取值写入，而不是在流程末尾凭记忆补一遍
整分钟。详细记录已在 `.code-review/evidence-defect-1008-manifest-timestamps.md`。
...
- recorder 视情况把「生成物同步性校验 ≠ 生成物正确性校验」补进 `lessons-learned.md`，
  与已有的那条讲「跑了但什么也没校验」的 job 比没有 job 更危险的规则相邻但独立列出。
- recorder 把「往既有文档追加内容前先照现有条目的形态写」补进
  `guides/push-and-pr-review-workflow.md` 或 `lessons-learned.md`（新条目 15）；这条
  在本次收尾阶段独立解决了一整类此前反复纠缠的碎行争议，具备跨任务复用价值。
- 若后续再跑 close-local-code-review 一类需要 manifest 记录事件时刻的流程，事件发生时
  当场用 `date -Is` 落盘，不要在流程末尾凭记忆补记整分钟；本条只记在反思里，不建议单独
  提升为 stable rule——它是本次流程执行纪律问题，不是可复用的技术判据。
