...
11. **引用统计数字前先确认量的就是所声称的那件事**：我曾用「汉字紧接西文／反引号的
    出现数重排前后一致（189 → 189）」来证明没有丢 CJK／西文之间的空格。把那 189 处
    逐字符拆开：`*` 66、`—` 58、`“` 23、`”` 22、`` ` `` 6、`>` 5、`/` 3、`\` 2、
    `}` 2、`…` 1、`,` 1——**拉丁字母 0 处**。那个字符类一次也没匹配到我声称在数的
    东西，数出来的几乎全是 Markdown 标记和标点；该计数在重排后还实际变成了 191，
    「前后一致」本身也不成立。按对应缺陷的口径重新核对（同一文件）：「汉字紧接拉丁
    字母」在基线与重排前为 0、重排后为 1（即被修的那处）、当前为 0。结论成立但证据
    与结论毫无关系——复核者按我给的字符类根本数不出 189。数字自洽不等于它在量你说的
    那件事。
