...
11. **引用统计数字前先确认量的就是所声称的那件事**：我曾用「汉字紧接西文／反引号的
    出现数重排前后一致（189 → 189）」来证明没有丢 CJK／西文之间的空格，而那个正则的
    字符类除拉丁字母和反引号外还漏进了 General Punctuation 区（`—`、`“`、`”`、`…`
    共 104 个），量的并不是所声称的那件事；重排后该计数实际是 191，「前后一致」也不
    成立。按对应缺陷本身的口径重新核对：「汉字紧接拉丁字母」始终为 0，含反引号是
    6 → 6，结论成立但证据是假的——复核者复现不出那个数字。数字自洽不等于它在量你说的
    那件事。
