...
11. **引用统计数字前先确认度量口径对应所声称的性质**：我曾用「汉字紧接西文的出现数
    重排前后一致（189 → 189）」来证明没有丢 CJK／西文之间的空格，而那个正则的字符类
    把全角标点也算作「西文」，量的其实是「汉字后跟任意非汉字」。按正确口径重新核对是
    0 → 0，结论成立但证据是假的——复核者复现不出那个数字。数字自洽不等于它在量你说的
    那件事。
