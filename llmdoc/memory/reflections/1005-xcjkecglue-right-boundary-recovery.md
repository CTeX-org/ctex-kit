...
- 已完成：PR #1005 合并后，从合并提交重新运行 #992 活表使用的矩阵，再把 #1003 对应红叉改成绿勾；不要在 PR 尚未合并时提前修改 issue 活表。
...
PR #1005 以 rebase 方式合并为 `master` `8007e4df`。从该提交重新解包 xeCJK 后，core、links、verb、biblatex 四组驱动分别运行 `xCJKecglue=false/true` 与默认／可区分间距，共 16 次编译；四种配置的普通命令均为 320／320 通过，日志中没有 `MATRIX|...|FAIL|`。重新生成的 `\mbox`、`\fbox`、`\null` 三张图片与 PR 预览图片逐字节一致。

据此，#992 活表第 7、9、15 行的 `xCJKecglue=true` 子行改为全绿，并刷新规范图片；第 28 行由 #1002 跟踪的公式红叉继续保留。这次收尾验证了“PR 中保存预览、合并后从合并提交更新活表”的分层做法：预览资产可以提前准备，但长期状态和规范图片必须以已合并提交为依据。
