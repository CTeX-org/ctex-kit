...
教训补充：**放宽一个模式时，要把该函数所有写入侧的实际形态都列出来逐个验证**，而不是只验证「原先被误拒的那些形态现在能过」。我这次只测了 `3.11a`／`0.0-beta`，没测 ctex——而函数自己的 docstring 正把 ctex 列为第二条分支的代表。

教训：**两侧对同一个值的「合法形态」必须有单一定义**。实践做法是先把写入侧做过的规范化步骤列出来（这里是「剥 `v` 前缀」「内容无约束」），再逐项确认校验侧对应处理；照抄同类分支时要逐项核对而不是整段复制。
