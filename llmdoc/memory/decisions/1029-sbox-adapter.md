...
`\savebox` 的三种形式（无可选参数、`[wd]`、`[wd][pos]`）最终都汇入同一个内部入口 `sbox `，因此同样受影响；`\global\setbox` 不受影响，因为 `\global` 直接贴在 `\setbox` 原语前面，中间没有钩子代码可以插入的位置。
...
- 新增回归 `xeCJK/testfiles/boundary-sbox-global01.lvt/.tlg`（6 项）：覆盖四种触发 `\global` 前缀的写法（`\global\sbox`、`\global\savebox` 三种形式）跨分组保住内容；不带 `\global` 的普通 `\sbox` 仍是局部赋值；嵌套 `\sbox` 的暂停深度可嵌套且成对归零；`\hbox{中\sbox\tb{english}x}` 与 `\hbox{中x}` 同宽（20.28pt）证明暂停观察语义不受影响。已用重新引入缺陷（还原为两个通用钩子）的方式确认四处 outside 尺寸会从正常值退化为 0.0pt（rc 1）。
