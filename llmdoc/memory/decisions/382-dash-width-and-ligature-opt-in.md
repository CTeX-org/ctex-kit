...
- 架构文档：`llmdoc/architecture/xecjk-architecture.md` 标点压缩系统一节（破折号宽度算法、`PoZheHao` 字符类、标点度量 feature-blind 限制），以及字符分类体系的特殊 interchar 类小节；#158 已证明单类零注入模板不能直接表达需要区分音节边界的 Hangul。
- 反思：[[../archive/2026-07-13/382-dash-width-punct-if-right-and-cmap-metrics]]。
