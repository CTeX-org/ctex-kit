...
## 已接受的限制：重排交还的那枚空格仍在片段盒内

重排路径把尾随空格从正文里剥离后再交还 `ulem` 排版。这枚空格拼在 `\l_@@_ulem_body_tl` 末尾，`ulem` 扫描到它时前面已无可切分内容，于是它落在最后一个正文片段盒**内部**，收缩量同样被固化。按深度汇总同一样例的外层收缩量：发布版 v3.10.3 为 9.44pt，回归后的基线 8.33pt，本次修复后 9.44pt 减 1.11pt 等于 8.33pt——即修复恢复了西文词那 2.22pt，但这枚空格自身的 1.11pt 仍在盒内。

尝试过改走已有的 `\@@_boundary_use_ulem_glue:n` 外层通道（`\UL@stop`／`\UL@start`）。外层收缩量确实回到与发布版一致的 9.44pt，但同时破坏 #1002 的边界契约：`command-boundary-math01` 报出 3.33pt 的 boundary delta 实测失败（`suffix-space/consumed-two-underline` 各形态），`command-boundary-math05` 的 `stream-ulem` previous 从 0.0pt 变成 3.33pt——这枚空格对边界机制变得可见，等于被计算两次。因此不采用。

判断依据：在 `\hsize` 180–260pt 的扫描中没有找到让这 1.11pt 改变断行结果或溢出量的取值，可观察影响为零；而修好它需要重新设计重排与边界契约的交互，超出 #1026 的范围。若将来要处理，应当连同 #1002 的 `math-space` 语义一并设计，并先补上能观察这 1.11pt 的用例。
