...
- capture 出口按入口前类别、实际首类别和源码空格重建左边界，再按实际末类别重放 marker 给普通右边界恢复链；重放 Default 类 marker 时同步外层列表当前的 `spacefactor`。
- 无可见输出时完整恢复入口状态。
...
| `post-transparent` | 只能用 after hook 处理的零尺寸末尾盒子；以真实 marker 为证据移动 `marker` 或 `marker + 一枚 glue` |
...
`\xeCJKsetup{experiment/boundary-register=...}` 允许专家用户直接把一个控制序列
交给上述五种注册策略。它是内部框架的受限入口，不是另一套恢复算法：`box`、
`wrapped-box`、`stream`、`transparent`、`post-transparent` 最终仍调用同一组
内部注册函数。前三种默认使用 `auto`；`box`／`wrapped-box` 还允许
`default`，`stream` 还允许 `default` 和 `first-default`；后两种不接受 `mode`。

用户声明全局收集，到导言区末尾、全部内建注册完成后统一应用；普通
`\AtBeginDocument` 代码执行完后再检查目标是否存在。接口只允许新增，不提供
反注册或覆盖。冲突判断同时查询通用注册表和专用适配器保留表，并在保存记录与
实际应用两个阶段各执行一次；否则导言区内后来加载的宏包，或 `\verb`、
`\Url@z`、ulem、listings 等不经过通用注册函数的专用入口，仍可能被错误叠加
普通命令 hook。完整生命周期和接口契约见
[[1010-boundary-register-public-api]]。

公开入口没有改变内部框架的机制边界。`auto` 只观察 capture 能看到的类别，不能
分析任意命令怎样消费参数；#1002 的行内公式支持来自各目标命令的参数适配器和
实际 math 节点确认，不是五种策略自动具备的能力。特殊扫描器也可能必须在真实
扫描入口直接适配，不能只给公开命令套一层通用 hook。
...
- Issues：#491、#991、#992、#995、#996、#998、#1000、#1003；PR #999、PR #1001、PR #1005
