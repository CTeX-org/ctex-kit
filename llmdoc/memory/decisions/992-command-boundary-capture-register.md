...
**math 与 rule 对 capture 已按 Default 重建（#998，PR #1001，commit 14336c4d）**：math 模式内容与 `\vrule` 仍不触发 XeTeX interchar class 转换，capture 依然观察不到首、尾类别本身；但 box/wrapped-box 结束路径新增 `\@@_boundary_if_capture_box_visible:`——在“无观察类别”前提下联合判定盒是否携带现场墨迹：宽度非零、且高度或深度非零，同时用 `\hbox_unpack:` 把该盒复制到临时盒读 `\tex_lastnodetype:D` 探测尾节点类型，命中 char(0)/rule(3)/math(10)/kern(12) 才视为“本层现场排版留下的痕迹”。命中时按 Default 首尾重建边界（`\mbox{$x$}`、`\mbox{\vrule...}` 现表现为与直接输入西文等价）；未命中（尾节点是 hlist(1)/glue(11)，或盒零尺寸/单一维度为零）保持完整透明恢复——这一保护集专为 `\null`/空 `\mbox`、ctex 内核 `\[` display leader 的空白 `\makebox` 占位盒、thuthesis `\thu@pad` 一类把预排盒/填充 glue 塞进末尾的命令而设，capture 无法判断它们内部的真实类别。推断出的 default 经专用函数 `\@@_boundary_capture_report_first:n` 上报——它只写尚未取得 `first_tl` 的外层 capture，绝不覆写外层已观察到的 `last_tl`，与「观察到实际类别」这条既有路径完全区分入口函数，从而保护 `\fcolorbox` 内层盒的 default 推断不降级覆盖外层已上报的 CJK 末类别，也保护 `\nfss@text`（`\ref` 内部 `\mbox`）嵌套在 stream 内时与裸调用等价。`\mbox{\vrule...}` 的契约 oracle 是 Default 字母（引擎对直接输入的裸 `\vrule` 本身不触发 interchar，这一条引擎机制边界不因本次修复改变）；`\fbox{$x$}`/`\colorbox{yellow}{$x$}` 的尾节点是 frame/wrapper hlist，仍保持透明恢复，留作 #992 已知项（透明保持集会因命令的包装节点形状而扩大）。回归 `command-boundary01` 新增 12 组场景（覆盖 mbox-math/mbox-mathdigit/mbox-digit/mbox-rule/mbox-empty 的中西双上下文），矩阵总数从 408 升到 456。
...
`command-boundary01` 用 114 组场景执行 456 个 direct-input 宽度 oracle（110 组普通矩阵 + 4 组显式 `\verb`），并逐单元断言 capture/active/suspend 状态归零；含五组原生 ulem 与 fntef 线型/符号命令双向嵌套（锁定“仅最外层拥有 stream”的不变量）、四组跨策略嵌套（transparent/box/wrapped-box 互相包含）与 PR #1001 新增 12 组 math/数字/rule/空盒场景（见上文 #998）。`command-boundary02` 用 12 个 paragraph/node 测试覆盖节点结构、ulem 外层非装饰 glue、显式 glue 歧义和 `\kern0pt` workaround。`listings-color01` 另执行 20 个 braced/delimited direct-input 比较。既有 xeCJK 回归（含 `colorbox-measure01`、PR #1001 新增 `boundary-crossbox01`/`siunitx-ecglue01`）、ctex 四引擎 184 项及专项测试共同锁定下游变化。

#992 的活表只代表已合并状态。PR 未合并时，新的矩阵结果只能作为 PR 预览；合并后必须从合并提交复验再更新 issue，本文档当前口径基于 PR #1001（4 个 commit：085f4f86/14336c4d/c8c803bf/a1df81ef）CI 全绿但尚未 merge 的分支状态。
