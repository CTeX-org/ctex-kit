...
**math 与 rule 对 capture 不可见（#998）**：math 模式内容与 `\vrule` 不触发 XeTeX interchar class 转换，capture 因而观察不到首、尾类别；box 策略把 `\mbox{$x$}`、`\mbox{\vrule...}` 判定为“无可见输出”并完整恢复入口状态，这与命令实际留下可见排版内容矛盾，违反上文输出等价契约。矩阵实测（中文·内容·中文，`00/10/01/11`）：math 场景在 v3.10.3 上四组全灭（`❌❌❌❌`，默认 glue 下每格 −6.66pt），#999 后仅 `11` 修复（`❌❌❌✅`）；rule 场景 v3.10.3 上 `✅✅❌❌`，#999 后 `00`/`10` 两组在可区分 `CJKglue` 下暴露出新的 +1pt 偏差，`11` 组失败形态从 `-3.33pt` 变为 `+3.33pt`（原偏差未消除，只是换了符号与幅度）。稳定 workaround：rule 用 `\mbox{...}\kern0pt\ `（v3.10.3 起有效）或改用原语 `\hbox`（#999 后起效）；math 侧 `11`（两侧都有源码空格）在 #999 后已正常，缺源码空格的一侧需各补一个 `~`（实测 `前~\mbox{$x$}~后`、`前 \mbox{$x$}~后`、`前~\mbox{$x$} 后` 均归零；只补左侧的 `前~\mbox{$x$}后` 右边界仍差 −3.33pt）。候选修复方向是让 box 策略在“盒非空但未观察到任何类别”时回退为 Default 首尾，但这会牵涉 `\null` 的透明语义，需单独评估，当前未采纳为待定项。此前 dtx 注释仅在 url 补丁历史里隐含提到 math 不触发 interchar；现明确记为 capture 框架的第二条已知机制边界，与上文“显式同构 glue”并列。
...
`command-boundary01` 用 102 组场景执行 408 个 direct-input 宽度 oracle（98 组普通矩阵 + 4 组显式 `\verb`），并逐单元断言 capture/active/suspend 状态归零；含五组原生 ulem 与 fntef 线型/符号命令双向嵌套（锁定“仅最外层拥有 stream”的不变量）与四组跨策略嵌套（transparent/box/wrapped-box 互相包含）。`command-boundary02` 用 12 个 paragraph/node 测试覆盖节点结构、ulem 外层非装饰 glue、显式 glue 歧义和 `\kern0pt` workaround。`listings-color01` 另执行 20 个 braced/delimited direct-input 比较。既有 xeCJK 回归（100 项既有 + 本轮新增 `colorbox-measure01` 共 101 项）、ctex 四引擎 184 项及专项测试共同锁定下游变化。
