...
首版把 `\changes` 记为 v2.6.2，但 `ctex-v2.6.2` 已于 2026-07-11 发布，#381
在 2026-07-13 才落地，因而必须归入下一个未发布版本 v2.6.3。错误来自只沿用
`build.lua` 的当前版本和 CHANGELOG 首节，没有先核对 release tag；修正后重新生成
CHANGELOG，v2.6.3 与已发布的 v2.6.2 分节保存。
...
- 写 `\changes` 前先核对最新正式 release；已发布版本之后的新变更必须进入下一个未发布版本，不能从 `build.lua` 当前值反推。
