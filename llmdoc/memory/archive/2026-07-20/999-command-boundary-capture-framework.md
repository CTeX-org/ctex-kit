...
- 宽度 oracle 回答“候选与直接输入的外围几何是否相同”；`command-boundary01` 最终用 372 个比较覆盖主体矩阵，包含原生 `\uline`；`listings-color01` 另用 20 个比较覆盖 `\lstinline` 的两类扫描入口。
...
只把包内 `\CJKunderline` 接入 stream 仍不足以替代旧 ulem 边界逻辑：原生 `\uline` 与声明式 `\xeCJKfntefon` 经过 `\ULon`，不会经过包内命令的分组入口。把 begin 直接下移到 `\UL@hook` 又太晚，此时已经进入内部字盒，会导致分组失配。最终保留包内命令的早期入口，并在 `\ULon` 用局部布尔补齐未启动的原生路径；公共结束点统一收束。
