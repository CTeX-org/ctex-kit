...
#491 的逐命令案例让失败看起来像不断增长的 edge case。#992 先把 26 行状态表展开为实际输出类别、相邻类别和 `00/10/01/11`，再用节点探针把失败归入盒子遮蔽、流式节点、透明节点和源码空格回卷等有限形状。只有先建立完整矩阵，才能判断公共原语是否真的覆盖问题族，而不是把某个 MWE 修绿。
...
- 宽度 oracle 回答“候选与直接输入的外围几何是否相同”；`command-boundary01` 用 360 个比较覆盖主体矩阵。
- 节点 oracle 回答“等宽结果是否由正确 glue/kern/box 结构产生”；`command-boundary02` 锁定 11 个段落与节点场景。
...
TeX glue 节点不携带“源码空格”或“显式 `\hskip`”来源。两者参数完全相同时，任何节点级算法都不可能可靠区分。受 pending 和可信 marker 限制的有界回卷已经是可证明的最大安全范围；继续向后猜测只会扩大误伤。

这类限制应同时提供机制证据、回归和 workaround。`\kern0pt` 放在同构显式 glue 前可阻止回卷，使用不同自然宽度或无 shrink glue 也可避开。明确承认不可区分，比把罕见输入静默解释成源码空格更可维护。
...
统一框架覆盖 box、wrapped-box、stream、transparent、post-transparent，处理实际首尾类别、混合输出、嵌套、`\sbox` 隔离、源码空格、`CJKspace` / `xCJKecglue`、hyperref、verb、hypdoc、biblatex 和一般 `\null`。#991 的 `\@setref` token 时序与少量第三方内部修正继续保留为窄特例；历史定点补丁成为演进证据，而不是未来新增 case 的模板。
