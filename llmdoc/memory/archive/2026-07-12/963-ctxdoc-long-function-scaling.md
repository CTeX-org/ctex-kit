...
压缩采用离散步长而不是一次缩到固定宽度：每轮减少 `0.15\marginparwidth`，直到宽度低于可用边注宽度。可用宽度总是扣除 `\marginparsep`，EXP 与 rEXP 再分别为右栏标记预留约 `1.2em` 与 `1.5em`，不可展函数不额外扣除。

#963 的讨论最初提出约 85% 的 hard limit，但实测发现把所有超长名称连续缩到同一宽度会让字宽变化过细、视觉区分不足，因此改成固定步长形成少量离散缩放档位。最终的 `1.2em` / `1.5em` 不是任意常数：l3doc 的 function 表在可展性标记前增加 6pt 列间距，实测 `$\star$` 加 6pt 约为 `1.2em`，`\ding{73}` 加 6pt 约为 `1.5em`；使用 em 近似可以随当前字号缩放。
