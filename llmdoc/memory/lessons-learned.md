...
### 多引擎包里，把用户输入的字符搬进标识符的方案要先问最弱那条路线
**Rule**: 凡是要用输入字符构造控制序列名、`\csname` 或变量名的方案，先确认这个字符在支持的**每一条**引擎路线上是什么形态。非 ASCII 字符在 pdfTeX 上根本不能进控制序列名。优先改为在数据层做规范化（把等价写法折成一种），这样各路线共用同一套后续逻辑。
**Why**: #1069 要支持直接输入 `ü`，第一版给它补表项 `\tl_const:Nn \c_@@_ü_tl`，XeTeX 下完全正确，pdfTeX 下**宏包加载即失败**（`Invalid UTF-8 byte "BC`）。改为在入口用 `\tl_replace_all:Nnn` 把 `ü` 折成 `v` 后两条线同时修好——该函数匹配记号序列，pdfTeX 下 `ü` 是 2 个记号也照样匹配（实测 `nü3` 4 个记号，替换后与 `nv3` 逐记号相同）。
**Source**: `llmdoc/memory/reflections/1069-pinyin-u-umlaut-input.md`

### 同一缺陷在不同引擎路线上症状可能完全不同，诊断要各做一次
**Rule**: 在多路线包里，复现缺陷不能只在报告者的引擎上做。输出错误、静默降级、编译中止是同一根因的不同表现；只看一条路线会低估严重性，也会漏掉那条路线特有的失败模式。测试覆盖两条线是一回事，**诊断**在两条线上各做一次是另一回事。
**Why**: #1069 在 XeTeX 下是「排出字面的 `nü3`」（输出错误），在 pdfTeX + CJKutf8 下是**编译当场中止**（`ü` 是两个字节，逐记号取到半个字符，`` `#1 `` 求值报 `Improper alphabetic constant`，rc 1）。issue 的截图只有 XeTeX，按它评估会把问题当成一处输出瑕疵。
**Source**: `llmdoc/memory/reflections/1069-pinyin-u-umlaut-input.md`

### 做输入规范化时要清点规范化后的值会流到哪些出口
**Rule**: 在入口把等价写法折成一种之后，逐一检查这个值的下游出口。参与计算的路径用折叠值；任何会把输入**原样回显**给用户的出口必须用原始值。
**Why**: #1069 折叠 `ü`→`v` 后，`\pinyin{nü}`（无声调数字）输出 `nv` 而基线是 `nü`——`\l_@@_save_tl` 是「整段没有声调数字」时的回显出口，我把它也设成了折叠后的值。这是实现修复时自己引入的回归。
**Source**: `llmdoc/memory/reflections/1069-pinyin-u-umlaut-input.md`

### 改完文件要验证产物，不能信脚本自己打印的「已改」
**Rule**: 用脚本批量改源码后，判据是 `grep` 生成产物（`.sty`/`.def`）或 `git diff`，不是脚本的输出。脚本里用绝对路径或开头显式 `cd`，不要依赖上一条命令遗留的工作目录。
...
### 一个值要存进两处时，除了「都写了」还要问「规范化约定是否一致」
**Rule**: 同一份数据写进多张表时，除确认每张都更新外，还要确认写进去的**拼写／格式约定**与各表既有内容一致。约定不一致的表会在读取侧产生两种结果，且不会报错。
**Why**: #1069 发现 `\setpinyin{女}{nü3}` 之后 `\xpinyinvalue{女}` 在 `tone=number` 下返回 `nü3`，而查询表通篇用 `v` 记 `ü`（`xpinyin.lua` 生成时即如此），`number` 形式的设计目的正是能直接馈回 `\pinyin`。这是 #550「`\setpinyin` 要同时改两张表」的延续：那次修「有没有写」，这次修「写进去的拼写」。
**Source**: `llmdoc/memory/reflections/1069-pinyin-u-umlaut-input.md`
...
**Source**: `llmdoc/memory/reflections/1037-ulem-word-front-ecglue.md`
**Why (#1069 又一次，形态更隐蔽)**: 为「`Ü` 折成 `V` 而不是 `v`」写的断言用的输入是 `\pinyin{Nü3}`——里面的 `ü` 是**小写**，走的是 `ü`→`v` 那条规则，根本碰不到 `Ü` 那条。断言名（`N-umlaut-3`）与注释（「大写声母：Ü 折成 V」）看起来都对得上，只有输入里那个字符不对，于是 `Ü`→`v` 的变异全绿。补上 `\pinyin{NÜ3}` 与 `\pinyin{NV3}` 的对照后，同一变异只让这一格变红。**检查方式是问「如果这条规则不存在，这个输入的结果会变吗」，而不是看注释写了什么。**
...
代价是分三次才清完：第一次只改了第一组的一份副本，第二次补齐该组另一份，第三次才在审查提示下按本规则全量检索——**而这条规则正是第二次写下的，却在同一批改动里没有对自己执行**。教训是写下规则之后要当场按它把同类问题一次扫完，而不是等下一轮审查逐个指出。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`

### 同一批里对同类问题只用一种做法，并逐个验证，不要只验证做对的那个
**Rule**: 一批改动里修同类问题时，所有命中项用同一种做法；然后逐个验证每一处，而不是验证一处就在提交信息里写「已逐一验证」。做法不一致时，错的那种往往正是没被验证的那种。
**Why**: #550 把两处行号引用改成检索锚点时用了两种写法：一处用 `\cs_new_protected:Npn \@@_reselect_CJK_font:`（唯一命中），另一处只写裸宏名 `\@@_reselect_CJK_font:`（在同一文件命中三处：调用、macro 条目、定义），而提交信息声称「已逐一验证锚点唯一命中」。同批另一处替换还因为先窄替换再宽替换而把宏名连写了两遍。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`

### 改完一个文件要通读它一遍，检索命中之外的同类问题往往就在相邻几行
**Rule**: 用检索定位并修完某类问题后，把每个被编辑的文件通读一遍再提交。检索只能命中你想到的形式；同一文件里同类问题的其他写法通常就在改动点附近，通读能发现检索漏掉的那些。
**Why**: #550 为同一类失效引用连做四轮清理，每轮都扩宽了判据，而每轮的遗漏都落在自己刚编辑过的那个文件里、距改动点仅十几到二十几行：一次是同组另一份副本，一次是同一小节相邻六组同形式引用，一次是相隔 23 行的同一目标引用。判据越写越宽并没有解决问题，通读才会。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`

### 引用文档某处内容时要按写下时的版本解析，不能按当前版本
**Rule**: 把旧的行号引用改写成标题或内容引用时，先 `git show <写入提交>^:<文件>` 取出写下时的版本，按那一版解析行号指向什么；按当前版本解析会得到另一个对象。
**Why**: #550 改写两处行号引用时按 HEAD 解析，得到的规则名与写下时所指的完全是两条，且与 `index.md` 里同一事实的表述冲突。同一文件里另外五处按写下时版本解析的改写全部正确——两种做法混在一批里，错的那种恰好没被验证。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`

### 检索类清理要先写下「什么算一处」，否则每轮都会以当时想到的形式为界
**Rule**: 做「把某类问题全部清掉」的清理前，先把命中形式穷举成一份可执行的判据并写进提交或文档：包括所有等价写法（带冒号的 `file:NNN`、不带冒号的 `NNN-NNN` 区间、「第 N 行」「约 LNNN」这类自然语言表述），以及全部搜索范围（含文档目录自身）。判据先定，再检索。
**Why**: #550 为同一类失效引用做了三轮清理，每轮都「按规则检索」过，每轮的判据都比上一轮宽一点：第一轮只改了触发它的那一份副本；第二轮把范围划成「本任务改过的非文档文件」，漏掉文档目录自身；第三轮判据只认带冒号的写法，漏掉不带冒号的区间与「约 LNNN」，而遗漏就在自己刚编辑过的同一个文件的相邻几行里。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`

### 定义检索范围时不要把自己改过的文件类别排除在外
**Rule**: 按规则做「全量检索」时，范围定义要覆盖本次改动可能影响的全部文件，包括文档目录自身。凡是自己插入或删除过行的文件，都可能推移别处对它的行号引用。
**Why**: #550 把检索范围定为「本任务改过的非 llmdoc 文件」，于是漏掉 llmdoc 内部的四处行号引用；其中 `index.md` 的一处正是本任务插入新条目推移的（71→72），成因与正在清理的那几组完全同类。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`

### 写下一条规则后，当场按它把同类问题一次扫完
**Rule**: 在 llmdoc 里新增一条「以后遇到 X 要做 Y」的规则时，立刻在当前仓库里按该规则做一次全量检索并修完所有命中项。规则与它的首次执行应在同一批改动里完成，否则留下的同类问题会显得像是「规则不管用」。
**Why**: #550 写下「行号引用会被自己的改动推移，要回查」这条规则时，只修了触发它的那一处，同一个文件里另外两组共四份同类失效引用留在原处，下一轮审查逐个指了出来。规则本身是对的，代价出在没有对自己执行。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`

### 提交信息里点名修了哪几处，就要逐处回读确认改动真的落在那里
**Rule**: 写「已修 A、B、C」之前，对每一处用 `git diff` 或直接读该行确认改动确实在那里。批量替换尤其危险：同一段文字可能在多处出现，脚本改中的是另一处，而你以为改的是点名的那处。
**Why**: #550 第五轮的提交信息点名说修掉了 `llmdoc/index.md` 里 `build-and-test.md` 条目上的「unpack 不报错」，实际 `git diff` 显示该文件本轮只改了另外两行，被点名那行 base 与 HEAD 逐字相同——我的替换命中的是另一个条目（`reflections/550-*.md`）里的同源文本。声称三处、实际两处，漏掉的恰是唯一被点名的那条。审查者逐处回读时发现了这个不一致。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`
...
### 后台 agent 的输出文件「没变化」不等于「还在跑」
**Rule**: 判断后台 agent 是否存活，不能看它的输出文件有没有变化——文件恒定既可能是在思考，也可能是早已死亡。没有完成通知就一律按「未产出结论」处理，不要在任何面向用户的产物（PR 描述、回复、报告）里写「正在跑」。要等结果就阻塞等待，不要一边推进一边猜。
**Why**: #550 启动的第四次全范围终审 agent 启动后即死亡，输出文件恒为 135 字节。我把「文件未变化」读成「仍在运行」，据此三次声称「第四次终审正在跑」，还把这句写进了 PR 描述。实际那轮从未产出任何结论——本地盲审的覆盖终点是 `dac5fa30`，其后两个提交只经过 PR bot review。是用户指出「哪有？现在没有任何 sub-agent 在跑着」才发现。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`

### 改文档时结论性的章节要跟着叙事性的章节一起改
**Rule**: 一份文档里既有「过程叙述」又有「结论清单」（决策的「接受的限制」、reference 的对照表）时，改动过程叙述后必须回头核对结论清单。结论清单最容易被当成事实直接引用，也最容易因为不在当下讨论的段落里而漏掉。
**Why**: #550 的决策文档里，星号去重的判据改过四版，我为此加了四节讲演变，却始终没改「接受的限制」那一节——它到收尾复核时仍写着第一版的说法（「只在结果是纯文本时去重，首字母不受 `tone` 影响」），而那已被后三版全部推翻。同轮还查出「共 11 个字」按任何口径都不成立（单音字表 7、多音字表 8 或 5），该数散落在 `.dtx` 注释、`.lvt` 注释、决策、反思、lessons 五处。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`
...
**Why**: #550 判断星号形式能不能去重，先按「`tone` 是不是 `mark`」判（太粗，把从不标调的声母也排除了），再按「命令加 `scheme`」判（仍漏：整个音节是鼻音时没有可标调的元音、韵母以 `v` 开头时也不标调，这两条回退运行时才知道，「呣」「嗯」「掠」「铝」等字因此仍不去重）。最终改为先跑一次 `\clist_gremove_duplicates:N`、列表变空就退回逐项输出。实现上两个坑：试探不装盒会把内容排到页面上（实测 `,,,,háng,...`）；列表用局部变量则结果被盒子挡住传不出来。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`
...
**Why**: #550 早先发现「声母不该带声调」（「中」的声母排成 `zh1`），在声母侧修好并写了注释，却没查韵母侧。终审发现整体为鼻音的音节（`m`／`ng` 一类）韵母为空串，与声调数字拼接后只剩一个裸数字，`tone=mark` 下页面上排出孤立的「2」——与 `zh1` 是同一形态的另一侧。四个回归文件对这一形态零覆盖，十二轮增量审查全绿。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`
