...
### 引用统计数字前先确认量的就是所声称的那件事
**Rule**: 把「某个计数在改动前后一致」当作证据写出来之前，先检查度量口径是否真的对应所声称的性质：正则的字符类、统计范围（全文／单文件／某几个文件）、按行还是按出现次数，任何一处偏差都会让数字与结论脱节。数字看着自洽不等于它在量你说的东西；口径写错时结论可能仍然成立，但证据是假的，复核者复现不出来。
**Why**: #1008 的一次提交声称「汉字紧接西文／反引号的出现数与重排前一致（189 → 189）」，用以证明重排没有丢 CJK／西文之间的空格。拆开那 189 处后，**其中拉丁字母 0 处**——那个字符类一次也没匹配到我声称在数的东西，数出来的全是 Markdown 标记和标点；该计数在重排后也并非不变。按对应缺陷本身的口径（汉字紧接拉丁字母）重新核对，结论确实成立，但那句统计与结论毫无关系。教训不在具体数字，而在「先确认口径，再引用数字」这一步被跳过了：此后连续两轮盲审都在这段统计叙述里找出新的不准确处，直到把它压缩成一条原则加一个可复现的关键事实才收敛。
**Source**: `llmdoc/memory/reflections/1008-zhnum-counter-options-expansion.md`

### 事实性陈述的更正以「全仓所有实例」为单位
**Rule**: 改一处计数、页数、文件名、函数名或机制解释后，用 `grep` 扫一遍该说法的所有变体再收工；**引用本仓库其他小节时，从目标文件里复制标题原文，不要凭记忆重述**——重述出的标题 `grep` 不到，读者无法跳转，而且这类偏差会在同一批改动里反复出现（#1008 连栽四轮：先按相对位置指代、再按正文措辞指代、再漏掉内层引号、再把「是不是设计限制」写成「这是不是限制」）；并把「历史记述」（记录当时事实，保留原值）与「当前事实」（必须统一）分开。同时算上本次改动本身会不会让该数字再变一次。
...
### 更正引用型陈述要用脚本枚举同类引用逐个核对，不能凭记忆 grep
**Rule**: 改一处「引用其他小节要复制标题原文」之类的问题后，用脚本把本次改动新增行里同类引用全部抽出来，逐个与真实标题比对；不要凭记忆定位单个词组去改，那只会改中被点名的那一处。这与「事实性陈述的更正以「全仓所有实例」为单位」互补：那条讲范围要覆盖全仓，这条讲**枚举手段**必须机械化而非凭记忆，两者缺一都会漏。
**Why**: #1008 中「引用其他小节要复制标题原文」这条规则在同一分支连栽四轮——先按相对位置指代（写「上一条」而那条实际在下方 88 行）、再按正文措辞指代、再漏掉标题内层引号、再把「是不是设计限制」写成「这是不是限制」；每轮都只改了被点名的那一处。真正止住它的是改用脚本把本分支新增行里所有「……」引用抽出来、逐个与真实标题比对。
**Source**: `llmdoc/memory/reflections/1008-zhnum-counter-options-expansion.md`

### 论证性叙述里每多一个精确数字，就多一处要维护、也多一处会出错的地方
**Rule**: 写论证性叙述（尤其统计口径类描述）时，精确数字只保留支撑结论所必需的那几个；能用一条原则加一个可复现的关键事实说清的，不要铺展成分解表——数字本身需要维护，且每一个都是独立的可错点。与「引用统计数字前先确认量的就是所声称的那件事」互补：那条讲口径要对准所声称的性质，这条讲数量要少。
**Why**: #1008 的一段统计叙述做了十一个精确数字断言，连续三轮盲审每轮都在其中找出新的不准确处（口径写错、分解加不起来 6+104≠189、单文件与多文件范围混用）；每轮补一版更细的数字后，下一轮又有新的可挑之处。真正收敛是把整段压缩成「一条原则 + 一个可复现关键事实」（那 189 处里拉丁字母 0 处）。
**Source**: `llmdoc/memory/reflections/1008-zhnum-counter-options-expansion.md`

### 往既有文档追加内容前先看现有条目的形态，照着写
**Rule**: 追加内容前先核对该文档现有条目的行宽、折不折行、字段结构和条目粒度；偏离既有形态会派生出一串与内容本身无关的问题（碎行、列宽争议、被渲染成假列表项等），且这些问题在 review 中会被反复挑出，掩盖真正需要讨论的内容。
**Why**: #1008 给 `lessons-learned.md` 新加的两条规则一度是全文唯一的折行段落（基线续行数 0，新增 12 行），此前反复纠缠的碎行、并回、列宽区间之争全部由此派生；按既有格式取消折行后整类问题一次消掉。另一实例是 `index.md` 里以 `+ ` 开头的续行被渲染成假列表项。
**Source**: `llmdoc/memory/reflections/1008-zhnum-counter-options-expansion.md`
...
**Why**: #1008 中 `\zhnumwithoptions`／`\zhdigwithoptions` 不可展开，最初用 `\typeout{\zhnumwithoptions{...}{...}}` 或 `\tl_set:Nx` 观察，都只记下命令名字；实测恢复 `\zhdigwithoptions` 多传一个参数的笔误后，只用这两种手段的测试版本仍然全绿。改成让它们实际排出汉字再量盒子高度后，这个笔误才第一次被测试拦住。这条与 #1043 反思里「探针静默先自证有效」同属一类——先确认观察手段本身有没有能力看到你要断言的东西。
**Source**: `llmdoc/memory/reflections/1008-zhnum-counter-options-expansion.md`
...
**Rule**: 「重新生成 + git diff」式的新鲜度校验只能证明前者，对**确定性**缺陷（生成脚本本身有 bug，两次生成结果一致但都错）零判别力——两边一致只是两边都错。需要另加一道校验产物内容本身是否正确。这与既有的两条同族——一条讲「跑了但什么也没校验」的 job 比没有 job 更危险，另一条讲对账／校验脚本本身也要做判别力实测——但那两条管的是校验**有没有跑起来**，这条管的是新鲜度校验这一**类型**本身结构性地看不见什么。
**Why**: zhnumber v3.2 的 CHANGELOG 条目里漏出过 `scripts/extract-changes.py` 的原始占位符控制字符（`\texttt{... \cs{???} ...}` 这类嵌套里内层占位符被整段收进 `verbatim_blocks` 后再也扫不到），而 `check-changelog.yml` 的「重新生成 + diff」新鲜度 job 一直是绿的——两边生成物逐字节相同，只是都错。修法是在新鲜度校验之外，另加一道直接检查产物内容（`CHANGELOG.md` 不得含 `\x00`–`\x05` 占位符）的校验。
**Source**: `llmdoc/memory/reflections/1008-zhnum-counter-options-expansion.md`
