...
**Why**: #1008 的一次提交声称「汉字紧接西文的出现数与重排前一致（189 → 189）」，用以证明重排没有丢 CJK/西文之间的空格。第四轮盲审复现不出 189——我用的字符类把全角标点也算作「西文」，于是量的是「汉字后面跟任意非汉字」。按正确口径（汉字紧接拉丁字母）重新核对是 0 → 0，结论确实成立，但那句统计本身是假证据。
**Source**: `llmdoc/memory/reflections/1008-zhnum-counter-options-expansion.md`
