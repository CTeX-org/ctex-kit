...
**Why**: 把代码事实归因成视觉错觉是这条失效的典型后果（#1046）。同一任务里还有第二个例子，而且它自己就栽在这条规则上：hyperref 的行内锚点确有两个出口（`\hyper@anchor` 与 `\Hy@raisedlink`），但最初据「注册一个不够」推断出的分派依据（非空目标走前者、空目标走后者）是错的——盲审用计数器包装两个命令实测发现四种 `\hypertarget` 形式的 `\Hy@raisedlink` 调用次数均为 0，真正的区分依据是调用点。「两个注册的判别力互不重叠」只证明有两个出口，不证明按什么分派；后者需要单独的探针。
**Source**: `llmdoc/memory/reflections/1046-1047-meta-anchor-font-context.md`

### 独立事实与联系它们的机制是分开的命题
**Rule**: 「A 缺了会坏、B 缺了也会坏」证明 A 与 B 都在路径上，不证明「按某个条件在 A 与 B 之间分派」。把若干独立事实叙述成一条机制时，那句机制陈述是需要独立证据的新命题——控制流问题要用针对控制流的探针（例如给两个候选函数各加 `\newcount` 计数器跑一次），不能从「都必要」推出「怎么分派」。准备把机制陈述提升为跨任务判据时更要先补这次实测。
**Why**: #1047 中两个 transparent 注册确实都必要、判别力也确实互不重叠，但据此推断的「非空目标走 `\Hy@raisedlink`、空目标走 `\hyper@anchor`」是错的：盲审加计数器实测发现四种 `\hypertarget` 形式的 `\Hy@raisedlink` 调用次数均为 0，两个分支都经 `\hyper@@anchor` 落到 `\hyper@anchor`；`\Hy@raisedlink` 的真实来源是目录、脚注与下游手工包裹。错误陈述已写进四份文档、其中两处被提升为通用规则，而推翻它只需一次编译。这与 #1043 的「事实、原因、后果各自都要验证」同源，多出的一层是「事实之间的联系」也要验证。
**Source**: `llmdoc/memory/reflections/1046-1047-meta-anchor-font-context.md`
