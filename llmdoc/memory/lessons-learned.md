...
**Why**: #1008 的一次提交声称「汉字紧接西文／反引号的出现数与重排前一致（189 → 189）」，用以证明重排没有丢 CJK／西文之间的空格。逐字符拆开那 189 处（重排前的 `lessons-learned.md`）：`*` 66、`—` 58、`“` 23、`”` 22、`` ` `` 6、`>` 5、`/` 3、`\` 2、`}` 2、`…` 1、`,` 1——**拉丁字母 0 处**。也就是说那个字符类一次也没匹配到我声称在数的东西，数出来的几乎全是 Markdown 标记和标点。而且该计数在重排后实际是 191，「前后一致」本身也不成立。按对应缺陷本身的口径重新核对（同一文件）：「汉字紧接拉丁字母」在分支基线与重排前为 0、重排后为 1（即被修的那处）、当前为 0；含反引号是 6 → 6。结论确实成立，但那句统计与结论毫无关系。
**Source**: `llmdoc/memory/reflections/1008-zhnum-counter-options-expansion.md`
