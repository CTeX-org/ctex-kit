...
**Why**: #1008 的一次提交声称「汉字紧接西文／反引号的出现数与重排前一致（189 → 189）」，用以证明重排没有丢 CJK／西文之间的空格。盲审复现不出 189——我用的字符类除拉丁字母和反引号外还漏进了 General Punctuation 区（`—` 58、`“` 23、`”` 22、`…` 1，共 104 个），于是量的并不是所声称的那件事；而且重排后该计数实际是 191，「前后一致」也不成立。按对应缺陷本身的口径重新核对：「汉字紧接拉丁字母」在分支基线、重排前、当前均为 0，含反引号则为 6 → 6，结论确实成立，但那句统计本身是假证据。
**Source**: `llmdoc/memory/reflections/1008-zhnum-counter-options-expansion.md`
