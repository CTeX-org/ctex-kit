...
**Why**: 把代码事实归因成视觉错觉是这条失效的典型后果（#1046）。同一任务里还有第二个例子，而且它自己就栽在这条规则上：hyperref 的行内锚点至少有三个出口，而我先后写错了两次——先误写按「目标是否为空」在 `\hyper@anchor` 与 `\Hy@raisedlink` 之间分派（计数器实测四种 `\hypertarget` 形式的 `\Hy@raisedlink` 均为 0），改对后又误写「只有两个出口」（同一手段随即找出 `\__hyp_target_raise:n`）。「两处注册的判别力互不重叠」只证明这两处都在路径上，既不证明按什么分派、也不证明只有两处。
**Source**: `llmdoc/memory/reflections/1046-1047-meta-anchor-font-context.md`

### 独立事实与联系它们的机制是分开的命题
**Rule**: 「A 缺了会坏、B 缺了也会坏」只证明 A 与 B 都在路径上。它**既不**证明「按某个条件在 A 与 B 之间分派」，**也不**证明「只有 A 和 B」。这三者是彼此独立的命题，各需自己的探针：控制流问题要用针对控制流的探针（给候选函数各加 `\newcount` 计数器跑一次），穷尽性断言要能说明用什么手段排除了第三种可能。准备把机制陈述提升为跨任务判据时尤其要先补这次实测。写下「全部」「两个」「只有」之前先自问一遍。
**Why**: #1047 的机制陈述被盲审连续推翻两次，形态相同——都是从「注册这些之后报告的现象消失」推出更强的断言。第一次：两个 transparent 注册确实都必要、判别力也确实互不重叠，但据此推断的「非空目标走 `\Hy@raisedlink`、空目标走 `\hyper@anchor`」是错的，计数器实测显示四种 `\hypertarget` 形式的 `\Hy@raisedlink` 调用次数均为 0。第二次：改对分派依据后又写成「行内锚点有两个出口」并把「两个」写进架构文档与本文件，同一手段随即发现第三个出口 `\__hyp_target_raise:n`（`\phantomsection`／`\MakeLinkTarget` 走它，`\phantomsection` 右侧仍缺 3.33pt）。两次错误都传播进了四到六份文档，而推翻各只需一次编译。这与 #1043 的「事实、原因、后果各自都要验证」同源，多出的两层是「事实之间的联系」与「事实的穷尽性」也要各自验证。
**Source**: `llmdoc/memory/reflections/1046-1047-meta-anchor-font-context.md`
