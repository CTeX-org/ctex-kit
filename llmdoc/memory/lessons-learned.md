...
**Why**: #550 的实现提交把 `\cs_new_eq:NN \@@_CJKsymbol_hook: \prg_do_nothing:` 从第 991 行推到 1240，于是 `build-and-test.md` 与 `decisions/997-*.md` 里两处「`xpinyin.dtx:990`」同时失效。我第一次只改了前者并把它记成「预存问题」，两点都错：逐提交追踪显示 990／991 在写下时是对的，是本任务自己推移的；而漏掉的第二份副本让仓库内同一句话互相矛盾。同批还在反思里写下日志行号「第 290 行」作证据，复现时同样不对——那次提交刚因为「行号会漂移」把源码引用换成锚点。
**Source**: `llmdoc/memory/reflections/550-xpinyin-pinyin-query.md`
