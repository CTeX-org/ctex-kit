...
**Why**: #1029 换掉的两个 `cmd/sbox` 钩子是 #992 为隔离 `\sbox` 离线测量而引入的。只看 #1029 自己的算法标题 MWE，无法说明隔离是否在换实现时丢了。补的 sbox 矩阵在 base 与修复后同为 96／96，而删掉 `suspend`／`resume` 的对照组为 72／96——有了这个对照，96／96 才是证据而不是空话。#997 是同一条规则的第二个实例，只是发生在更早的阶段：改动前就先回放了 hook 当初为之而生的场景（调用处当前字体是西文时，量宽盒子必须切到 CJK 字体），据此判定这段代码不能删，修复形态只能是给它加一个条件。
...
### 修复形态是「加一个条件」时，条件失效与条件写反是两种失败形态，各要一组用例
**Rule**: 上一条要求的「还原到修复前」只覆盖一个方向。若修复不是改写实现、而是给原有行为**加一个条件**（原逻辑在条件不成立时仍要执行），那么它有两种失败形态：条件不起作用（退回原缺陷）与条件写反（该执行时也跳过，属过度修复）。两者的症状不同，因此要分别有一组用例，并各自实测到对应的读数；只测一个方向时，另一个方向可以整套全绿。判断有没有覆盖第二个方向的办法很直接：把条件真假取反，跑一次看有没有用例变红。
**Why**: #997 的修复是「已处于后备字体状态就跳过字体重选，否则按原逻辑重选」。回退成无条件重选（原缺陷）时基线出现 16 处 `x2.8` 并带 `Missing character`；把条件写反成总是跳过时基线出现 `x0.0`（在西文字体下量宽）。第 1、2 项只固定前一种，第 3 项（主字体自带 CJK 字形、紧跟在西文之后）专门固定后一种——缺了第 3 项，「总是跳过」这个变异不会被任何用例发现。
**Source**: `llmdoc/memory/reflections/997-xpinyin-fallback-measure-box.md`
