...
新分支第一次 push 尚无 PR 时无法完成状态检查；PR 建立后立即运行 `make check-pr-ci 2>&1`。agentic review 的全部 finding 都须处理；若核实无需代码改动，OWNER/MEMBER/COLLABORATOR 在 bot 评论之后回复证据，再手动运行同一命令即可终止，不创建空 commit。完整闭环见 `llmdoc/guides/push-and-pr-review-workflow.md`，仓库决策见 `llmdoc/memory/decisions/repo-push-hook-discipline.md`。
...
前后对比和零缩进兼容语义 MWE/PDF）、`issues/995|996|998/`（#992 框架
A/B 复现的 MWE `.tex`、确认/对比 png 与 README）。前两批分别取代了此前的临时分支
