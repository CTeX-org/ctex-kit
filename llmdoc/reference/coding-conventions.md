...
`xeCJK/xeCJK.dtx` 的实现代码必须位于 `macrocode` 环境内；实现说明应先结束
`macrocode`，写完说明后再重新开始。这样既让 docstrip 继续抽取真实代码，也避免
`l3doc`／`ctxdoc` 把说明或实现误当成另一种内容。`macrocode` 的结束标记还必须严格写成
四个空格后的 `%    \end{macrocode}`：`support/ctxdoc.cls` 会按这个完整行模式构造结束标记，
空格多一个或少一个都不会被识别，随后通常在后文以 quark 相关错误结束文档构建。

实现代码虽然不影响运行时语义，但会原样排进手册。长控制序列名或动态构造的变量名应按
expl3 的分组层次换行，不能依赖页面自动折行；否则 `macrocode` 会越出手册版心。

用户文档和实现说明启用了 shortvrb，竖线 `|...|` 只适合不含特殊参数记号的短字面内容。
特别是 `\changes` 正文中的 `#1` 等参数签名，应使用 `\texttt{[\#1]...}`，不要放进
