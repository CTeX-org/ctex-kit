...
因此：每个报错场景各占一个 `.lvt`，且该文件里报错断言之后不放别的项；需要与排版类判据共存的项，放在不含报错断言的文件里。#550 的分法是 `pinyin-query01`（取值、切分、可展开性、`tone=mark` 节点基线）、`pinyin-query02`（参数校验报错）、`pinyin-query03`（查不到读音报错），三者的变异各自变红。
