...
`fntef-actualtext01`、#1012 新增 `fntef-phase01` 后，当前为 113／113 通过。完整接口
...
常见全角 CJK 字体和字重在同字号下通常不改变一 em 字宽及 leaders 几何，主要影响异常是否醒目；字号、非一 em 字宽、标点、特殊盒子和实际伸缩胶水则会改变片段宽度或余数。因此，自动回归不必复制完整字体矩阵，但必须覆盖真实字号、单元比例和实际使用伸缩量的断行；视觉抽样再加入 Serif／Sans、Regular／Black 等少量对照。xeCJK 标准测试当前为 113 项。
...
合同测试要求这两个准备 step 逐行使用同一份受控脚本，固定两条 sysctl 命令、完整探针参数及其
顺序；仅保留被注释的命令文字，或在续行反斜线后加入转义空格，都不能通过。
...
PR Review 不能让 Codex 把 head checkout 当作项目指令根。Codex 0.145.0 的 `--ignore-rules` 只禁用 execpolicy `.rules`，不会禁用 `AGENTS.md` 发现；仅在提示词中声明“仓库指令是数据”也不能覆盖 CLI 自动注入的项目指令。Codex 应从 session 内新建的空目录启动，再把 PR checkout 作为显式附加目录；提示词使用绝对路径指向待审仓库和私有目录中的可信规范。Claude Code 2.1.148 使用 `--bare` 明确禁用 `CLAUDE.md` 自动发现，不应再用会显式引入 `CLAUDE.md` 目录的 `--add-dir` 来追求形式一致。

模型生成的命令与 CLI 控制面虽然使用同一 UID，但不能共享结果文件的写权限。Codex 必须使用 `workspace-write` 且不得请求额外权限；Claude 必须启用命令沙箱、禁止 unsandboxed command，并在沙箱不可用时失败。最终结果只写到 `/run/ctex-agent-session.*/control/`，不写 Agent HOME 或 `/tmp`。可信 Action 编译的预加载库在 CLI 每次 `exec` 后设置 `PR_SET_DUMPABLE=0` 和 `PR_SET_NO_NEW_PRIVS=1`，阻止同 UID 子进程通过 `/proc/<pid>/fd` 或 ptrace 访问 CLI 控制面。CLI 返回后先反复结束该 UID 的全部后台进程，确认无人存活再读取结果；session 目录由 `sudo rm -rf` 清理。只把 `pkill` 移到读取前仍不能证明文件最后写入者，JSON Schema 也只能检查形式，不能替代这些来源边界。
...
`scripts/test-agentic-workflow-contract.py` 固定触发、权限、六处工具安装、restore/save 分离、可信的 Agent 启动前显式保存、事件提交、publisher 隔离和结构化结果语义；它还用预期失败的错误样例验证零 finding 的 `COMMENT`、损坏的 `runs.using`、拼错的 composite step 字段、由 `assume-unchanged` 隐藏的脚本改写、恶意 `core.fsmonitor` 会被 Git 执行、字体 staging 中预置或不完整的内容、同／异 head 评论发布、第二页 Bot 评论、维护者回复早于 Bot `updated_at`、工作区进程持续改写沙箱外控制结果，以及同 UID 子进程访问父 CLI `/proc/<pid>/fd` 的情况。PR Review 的合同还要求 Codex 从空工作根启动、把 head checkout 作为附加目录，Claude 保留 `--bare`，并把 base 固定的两份规范复制到 Agent 不可写的私有目录；恢复 Codex 以 PR 根目录启动、使规范可写或让提示词读取 `.trusted-base` 的反例都必须失败。llmdoc 通知也必须区分公开结果中的 `blocked` 与 job 执行成功。PR Review 的可信 sparse checkout 还要覆盖 `run-agent` Action 的全部仓库内运行时依赖；合同测试从 Action 中的 `$GITHUB_ACTION_PATH` 引用推导文件闭包，分别检查 Codex 与 Claude 的清单，并用删除 C 源文件路径的反例确认门禁会失败。新增或移动本地 Action 的运行时文件时，必须同时更新所有固定提交 checkout，不能只修改 Action 本身。`scripts/validate-action-metadata.py` 专门校验 `action.yml`，因为 actionlint 只负责 workflow。它的 composite step 字段表必须与 GitHub 实际支持的字段一致：`timeout-minutes` 只在 job step 合法，写进复合 Action 会让 runner 在加载 `action.yml` 时报 `TemplateValidationException` 并使整个调用步骤失败，因此校验器把这类仅 job 支持的字段单独判为错误，合同测试也用注入 `timeout-minutes` 的反例确认门禁会失败。工具安装步骤的挂起只能由调用 job 的 `timeout-minutes` 兜住。复合 Action 的 `run` 还有一处与 job step 不同的默认值：它的 shell 是 `bash -e -o pipefail`。因此 `tlmgr conf | awk '...;exit}'` 这类在管道右侧提前 `exit` 的写法，会让左侧进程收到 SIGPIPE，整条管道以 141 结束并终止安装步骤；同样的写法在不带 `pipefail` 的 job step（例如 `_check-doc-package.yml`）里是安全的。合同测试因此禁止复合 Action 在管道右侧使用提前 `exit` 的 awk；取值时应让 awk 读完全部输出后再输出结果，不要用 `exit` 提前终止。合同 workflow 的 `pull_request.paths` 必须覆盖合同测试读取或执行的全部仓库文件；例如测试直接读取 `.githooks/check-pr-ci.sh`，该 hook 就必须能触发门禁。独立 shell 文件还要由明确的 ShellCheck 命令检查，不能把 actionlint 对 workflow 内嵌 `run:` 的检查当作替代。修改本地 Agent runtime 后运行这两个脚本、actionlint 和 ShellCheck。设计与教训见 [[1025-agentic-local-runtime-toolchain]]、[[1030-1031-composite-action-semantics]]。

`agentic-pr-review.yml` 由 `pull_request_target` 触发，其工作流定义本身取自 base 分支（`master`）当前状态；但用于可信 checkout 的 `github.event.pull_request.base.sha` 是该 PR 的分叉点（merge base），不是 base 分支当前 HEAD。因此 Agent runtime（本节描述的三条 workflow、复合 Action 与 `.github/scripts/agentic/`）发生改动后，所有分叉点早于该改动的存量 PR 都会持续加载旧运行时，其 Agent job 会在可信 checkout 或工具安装阶段反复失败，直到该分支 rebase 到 `master` 或合并 `master` 为止；close/reopen PR 与单独重跑都不会改变分叉点，因此都不能恢复。这是刻意的安全设计：保证可信运行时的版本与被审查的 diff 有一致基线，代价是运行时改动不会对已存在、分叉点落后的 PR 自动生效。诊断步骤见 `llmdoc/guides/push-and-pr-review-workflow.md`。设计与教训见 [[1030-1031-composite-action-semantics]]。
