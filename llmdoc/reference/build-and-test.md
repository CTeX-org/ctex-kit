...
`boundary-crossbox01.lvt` 锁定 #996：跨盒 pending 生命周期（`\@@_glue_check_expire_stale:` 在顶层空列表探测时结束过期 `\g_@@_glue_check_pending_bool`，阻止其跨 `\hbox`/`\setbox` 存活）、同盒内/跨盒两种显式同构 glue 歧义窗口、`\kern0pt` workaround，以及双向 source-space 完整性，共 7 个 `\TEST`。
