...
- 118 组边界场景各跑四种源码空格，共 472 个比较；其中 114 组走 `\BoundaryMatrix`（含 ulem/fntef 双向嵌套、transparent/box/wrapped-box 跨策略嵌套、PR #1001 新增 12 组 math/数字/rule/空盒场景，以及本地 review 后补充的 4 组“已观察 CJK 前缀 + 推断 math/rule 后缀”嵌套场景），4 组分隔符扫描 `\verb` 用显式调用。测试先扣除候选命令与 oracle 单独排版的固有宽度差，再只比较外围边界，容差为 0.01pt。
...
`boundary-crossbox01.lvt` 锁定 #996：跨盒 pending 生命周期（`\@@_glue_check_expire_stale:` 在顶层空列表探测时结束过期 `\g_@@_glue_check_pending_bool`，阻止其跨 `\hbox`/`\setbox` 存活）、同盒内/跨盒两种显式同构 glue 歧义窗口、`\kern0pt` workaround，以及双向 source-space 完整性，共 8 个断言（7 宽度 + 1 pending 状态）。与 `command-boundary01` 的 `\BoundaryReset` 一样，测试 harness 直接读写 `\g__xeCJK_last_node_tl`、`\g__xeCJK_glue_check_pending_bool` 两个内部变量做白盒隔离与生命周期断言；它们不是公开 API，内部重命名时这些 `.lvt` 必须同步。

`siunitx-ecglue01.lvt` 锁定 #1000：`\unit`/`\qty`/`\num`/`\si`/`\SI` 五个命令 × 中文/西文上下文 × `00/10/01/11` 四种源码空格（`\BoundaryMatrix`），加 `\unit` 可选参数变体与 math 内嵌用法下 capture 栈归零断言，共 36 次宽度比较；预热段（`\OMIT`/`\TIMO`）先消化 siunitx 数学字体加载与旧名 deprecation 消息，避免污染规范化日志。
