...
**可展开的报错会中断后续断言，必须一个场景一个文件。** `\msg_expandable_error:nnn` 在 `\edef` 里发出错误后，当前 `\edef` 与其后的断言都不再执行。#550 把两个报错场景（参数不是单个字符、查不到读音）放在同一个 `.lvt` 里时，只有前一个有效，后一个的变异照常全绿——两者相互掩盖判别力。因此拆成 `pinyin-query02.lvt`（参数校验）与 `pinyin-query03.lvt`（查不到读音）各占一个文件，变异实测各自变红。
