...
- `llmdoc/memory/decisions/1012-fntef-decoration-overlap.md` — 决策：默认波浪与斜线使用随字号缩放的 `l3draw` `1em/4` 图案和普通 `\leaders` 共享相位；首末局部 PDF 裁切分别给普通／带减号形式提供对称的外伸／内缩范围，可断点两侧的半周期连接保持不断行与换行两种情况的接点语义；空参数和不产生节点的正文保持零尺寸；斜线约高 `.93em` 并下移 `.09em`，自定义 `underwave/symbol` 保留 `\xleaders`。
...
- `llmdoc/memory/reflections/1012-fntef-decoration-overlap.md` — 反思：#1012 先后否决清零 `\UL@pixel`、`1em/3` 加原 leaders 选择、朴素普通 `\leaders` 和 `\cleaders` 加胶水专用图形等不完整方案，保留普通 `\leaders` 的共享相位能力，再以首末裁切和断点两侧半周期连接补齐端点与断行接点；记录历史大字号重黑体验证的盲点、斜线纵向覆盖、实际加载路径核对，以及真实绘图度量、轻量节点替身、PDF 坐标门禁、精确视觉 MWE 和整本文档构建的分工。
