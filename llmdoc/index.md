...
- `llmdoc/architecture/xecjk-architecture.md` — xeCJK 独立架构详解：
  - interchar token、字符分类（#158/#165/#336/#347/#382）、基础 marker/glue 恢复链、#992 capture/register 策略与直接输入 oracle、嵌套/`\sbox` 隔离、显式 glue 判源边界和 `\kern0pt` workaround（含 PR #1001 已对称落地的 Boundary→CJK/Default 双向同构 glue 校验 #996、box/wrapped-box 盒尺寸+尾节点墨迹联合判定按 Default 重建 #998 两条机制记录），以及 #991 `\@setref` 特例被 auto stream 取代后的分工与字体管理（#553）；
...
- `llmdoc/reference/build-and-test.md` — `l3build`、共享构建配置、根 `Makefile` 本地任务入口（#888，含 `make changelog` #961）、`ctex` 184 个主回归测试的覆盖簇，以及 xeCJK #992 命令边界的三层证据：`command-boundary01` 的 114 组场景 × 4＝456 个宽度 oracle 与 idle-stack 断言（含 PR #1001 新增 math/rule/空盒场景 #998）、`command-boundary02` 的 12 个 paragraph/node 用例、`listings-color01` 的 20 个逐格 oracle、`colorbox-measure01`（#995 状态隔离回归）、`boundary-crossbox01`（#996 跨盒 pending 生命周期）、`siunitx-ecglue01`（#1000 单位命令边界）、#991 `ref-ecglue01/02` 的 76 个比较和 gh-assets 可视 MWE；另含显式 glue 判源边界、`\kern0pt` workaround、lazy family 预热、MWE 说明层隔离、多引擎基线、ctxdoc 专项回归、CI/CD、版本/生成物门禁与本地 TL usertree 同步。
...
- `llmdoc/memory/decisions/992-command-boundary-capture-register.md` — 决策: #992/PR #999 用 box、wrapped-box、stream、transparent、post-transparent 五类注册策略和运行时首尾类别统一恢复命令两侧边界；记录嵌套/`\sbox` 状态、旧逐命令补丁吸收结果、显式同构 glue 的机制边界与 `\kern0pt` workaround；现含 #995（已由框架顺带修复）、#996/#998/#1000（均已由 PR #1001 修复，活表待 merge 后复验）的 A/B 复现结论与修复实现。
...
- `llmdoc/memory/reflections/1001-boundary-capture-gap-fixes.md` — 反思: PR #1001 在 #999 capture/register 框架上收敛 #992 三个跟踪项——`l3build check -q 2>&1 | tail` 再次掩盖 6 个测试回归的真实 rc；#998「无可见输出」判据从单看观察类别，历经三轮加入盒尺寸与尾节点墨迹探针（char/rule/math/kern 才算现场排版）方才收敛，ctex 内核 `\[` display leader 与 thuthesis 预排盒是暴露反例的最快信号源；`report_first` 与覆写 `last_tl` 因共享同一函数入口一度让 fcolorbox 回归，拆分专用函数后解决；boundary-space02/fntef-space02 旧基线本身是待修 bug 而非受保护行为；gh-assets 992 矩阵驱动引用 #999 已删除的 `\g__xeCJK_reset_color_pending_bool` 需 `\bool_if_exist:NT` 守卫才能跨版本复用。
