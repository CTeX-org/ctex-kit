...
- `llmdoc/overview/project-overview.md` — 项目范围、仓库组织、核心/卫星包分类、技术栈与维护状态；现含三条本地 Agent workflow、六个 Agent job 的共享排版工具链、Agent 启动前可信缓存填充，以及 Agent 无外部写权限／独立 publisher／模型凭据代理的职责划分。
...
- `llmdoc/reference/build-and-test.md` — `l3build`、共享构建配置、根 `Makefile` 本地任务入口（#888，含 `make changelog` #961）、`ctex` 185 个主回归测试，以及 #994 `fontset-macnew01` 对生成配置、macOS 实际字形和 SPA 度量的分层验证；还含 xeCJK #992/#1002 命令边界测试：直接输入 oracle、`00/10/01/11`、`xCJKecglue=false/true`、默认/可区分间距与状态归零断言；`command-boundary01` 当前执行 1668 个绿色单元，`command-boundary-math01` 执行 5504 次公式比较，`command-boundary-math05` 固定尾随空格的伸缩量和不可见节点边界；#1010 的 `boundary-register-api01/02` 另以 288 项行为比较和诊断回归固定公开注册入口；#1012 将 fntef 绘图验证分为真实图形度量、同尺寸轻量节点替身、XDV→无压缩 PDF→Lua 坐标门禁、精确视觉 MWE 和整本文档集成构建，并要求核对实际加载的宏包路径；#1017 的 `fntef-actualtext01` 覆盖八类装饰的空 `ActualText` 与 tagging 暂停／恢复，并要求普通／tagged PDF 文本提取和页面视觉分别验收。xeCJK 标准测试当前为 113／113。文档还记录节点测试、显式 glue 的来源限制、`\kern0pt` 处理方法、字体预热、多引擎基线、CI/CD、本地 Agent runtime／工具链／凭据与 publisher 合同、Agent 启动前的可信缓存保存、字体缓存完整性、按 head 发布审查评论、完整分页和以 `updated_at` 判断维护者回复、版本检查和本地 TeX Live usertree 同步。
...
- `llmdoc/memory/lessons-learned.md` — 从已归档反思提炼的跨任务规则；当前含正式审查的新会话、受限输入和报告留痕门禁、生成物门禁、按风险收缩验证、平台条件测试的陈述边界、字体选择／映射／度量同步、节点/视觉证据、周期装饰的接点分工、源码树实际加载路径核对、PDF 绘图的尺寸／节点／坐标／外观分层回归、稳定文档随实现演进复核、源码候选须由实际输出确认、弹性 glue 的三项参数与实际断行、命令边界输出等价矩阵、绿色单元与失败基线分离、机制抽象与支持边界、说明层隔离、字体预热、PR 预览与主线事实分层，以及 feature request 和 git 测试规则。
...
- `llmdoc/memory/reflections/1025-agentic-local-runtime-toolchain.md` — 反思：将三条远端 reusable Agent workflow 展开到本仓库，为六个 Agent job 安装排版工具链；记录从“缓存绝对只读”修正为 Agent 启动前可信显式保存，并以字体完整性检查、专用用户、模型代理、固定事件提交、独立 publisher、按 head 的评论幂等性、完整分页和 `updated_at` 回复边界收紧运行合同。
...
- `llmdoc/memory/reflections/1012-fntef-decoration-overlap.md` — 反思：#1012 先后否决清零 `\UL@pixel`、`1em/3` 加原 leaders 选择、朴素普通 `\leaders` 和 `\cleaders` 加胶水专用图形等不完整方案，保留普通 `\leaders` 的共享相位能力，再以首末裁切和断点两侧半周期连接补齐端点与断行接点；记录历史大字号重黑体验证的盲点、斜线纵向覆盖、嵌套状态必须按调用层拥有、实际加载路径核对，以及真实绘图度量、轻量节点替身、PDF 坐标门禁、精确视觉 MWE 和整本文档构建的分工。
