...
- `llmdoc/architecture/xecjk-architecture.md` — xeCJK 独立架构详解：
  ...
  - 兼容补丁（#510、#873/#880/#910/#919/#931/#972）、`\char` 约束及扩展子包；xeCJKfntef 部分现含 #1012 的接点分工：默认波浪与斜线使用随字号缩放的 `l3draw` `1em/4` 图案和 `\cleaders`，分别处理正文片段、`CJKglue` 与普通／带减号形式的端点，自定义波浪符号保留 `\xleaders`；另含 #1017 对装饰盒使用空 `ActualText` 并在最小范围暂停 tagging 的 PDF 文本语义隔离。
...
- `llmdoc/memory/decisions/1012-fntef-decoration-overlap.md` — 决策：默认波浪与斜线使用随字号缩放的 `l3draw` `1em/4` 图案和 `\cleaders`；正文片段、`CJKglue` 与命令端点分别处理，普通形式左右各有半单元端点，带减号形式落在正文边界；波浪用水平线连接胶水两侧的波峰，斜线只保留实际胶水宽度，自定义 `underwave/symbol` 保留 `\xleaders`。
...
- `llmdoc/memory/reflections/1012-fntef-decoration-overlap.md` — 反思：#1012 先后否决清零 `\UL@pixel`、`1em/3` 加原 leaders 选择和普通 `\leaders` 三条中间路线，转为分别处理正文片段、伸缩 `CJKglue` 与命令端点；记录历史大字号重黑体验证的盲点、实际加载路径核对，以及真实绘图度量、轻量节点替身、PDF 坐标门禁、精确视觉 MWE 和整本文档构建的分工。
