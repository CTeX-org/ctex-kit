...
- `llmdoc/architecture/xecjk-architecture.md` — xeCJK 独立架构详解：
  - interchar token、字符分类（#158/#165/#336/#347/#382）、基础 marker/glue 恢复链、#992 capture/register 策略与直接输入 oracle、嵌套/`\sbox` 隔离、显式 glue 判源边界和 `\kern0pt` workaround（含 PR #1001 已对称落地的 Boundary→CJK/Default 双向同构 glue 校验 #996、box/wrapped-box 盒尺寸+尾节点墨迹联合判定按 Default 重建及可信尾 marker 逐层校正末类别 #998），以及 #991 `\@setref` 特例被 auto stream 取代后的分工与字体管理（#553）；
...
- `llmdoc/memory/reflections/1001-boundary-capture-gap-fixes.md` — 反思: PR #1001 在 #999 capture/register 框架上收敛 #992 三个跟踪项——`l3build check -q 2>&1 | tail` 再次掩盖 6 个测试回归的真实 rc；#998「无可见输出」判据加入盒尺寸与尾节点墨迹探针后，本地 review 又发现“已观察前缀 + 推断后缀”的嵌套盒未传播末类别，最终以盒内可信尾 marker 逐层校正，不能无条件覆写所有外层 `last_tl`；boundary-space02/fntef-space02 旧基线本身是待修 bug；gh-assets 992 矩阵驱动引用已删除内部变量时需存在性守卫。
