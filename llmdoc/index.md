...
- `llmdoc/architecture/xecjk-architecture.md` — xeCJK 独立架构详解：
  ...
  - 兼容补丁（#510、#873/#880/#910/#919/#931/#972）、`\char` 约束及扩展子包；xeCJKfntef 部分现含 #1012 的接点分工：默认波浪与斜线使用随字号缩放的 `l3draw` `1em/4` 图案和普通 `\leaders` 共享相位，以首末局部裁切控制普通／带减号形式的可见范围，并在可断点两侧各放半周期连接；斜线约高 `.93em` 并下移 `.09em` 以覆盖常见汉字的高度和深度，自定义波浪符号保留 `\xleaders`；另含 #1017 对装饰盒使用空 `ActualText` 并在最小范围暂停 tagging 的 PDF 文本语义隔离。
...
- `llmdoc/memory/decisions/1012-fntef-decoration-overlap.md` — 决策：默认波浪与斜线使用随字号缩放的 `l3draw` `1em/4` 图案和普通 `\leaders` 共享相位；首末局部 PDF 裁切分别给普通／带减号形式提供对称的外伸／内缩范围，可断点两侧的半周期连接保持不断行与换行两种情况的接点语义；斜线约高 `.93em` 并下移 `.09em`，自定义 `underwave/symbol` 保留 `\xleaders`。
