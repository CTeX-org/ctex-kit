...
- `llmdoc/architecture/package-architecture.md` — `ctex` 与 `xeCJK` 的主干架构、引擎适配策略、第三方包补丁子系统与包间依赖图；现含 xeCJK 对 #407/#800 的 `\xeCJKchar` + 定点补丁策略，以及边界恢复链中 `\lastkern` 标记 kern、whatsit 定点重放（含 `\set@color` / `\reset@color` / `\Hy@BeginAnnot` 三处）、#324 宏路径多余 glue 遮蔽、#826 fntef 右侧 glue-on-kern-pair 遮蔽、#770 显式 `}` / `\textcolor` color-pop / `\mbox` hlist 三种变体、ecglue 缓存取值四层约束的统一心智模型。
...
- `llmdoc/memory/decisions/770-boundary-explicit-brace-ecglue.md` — 决策: #770 显式 `}` / `\mbox` / `\textcolor` 右侧多余 inter-word glue 的三阶段修复：CJK→Boundary handler catcode 2 boolean set 点、`\reset@color` 定点补丁、hlist 回退路径；`\@@_check_for_glue_skip:` 重构为 kern/hlist 双路径。
