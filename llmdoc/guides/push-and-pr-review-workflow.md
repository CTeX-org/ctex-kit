...
正式的本地代码审查必须由上下文隔离的子代理（sub-agent）完成。这里的“上下文隔离”是指启动一个不继承主代理设计与实现对话的新子代理，例如使用 `fork_turns=none` 或等效的新会话。主代理只能向审查者提供仓库规则、任务或 PR 的公开目标、固定的完整 base/head SHA、审查范围和必要的公开证据；不能提供自己的实现推理、预期结论或审查答案。增量审查可以读取上一份正式报告，以取得审查截止点并避免重复，但仍不能继承主对话。审查报告必须记录采用的隔离方式、允许输入和固定提交范围，使维护者能够复核这三个条件。
...
`git diff --check`、必要的定向 MWE，并由 PR 强制 CI 完成跨平台验证。若后续改动触及
运行时、文档解析边界、示例语义、生成物或测试基线，再升级到完整本地检查。
...
复杂的可见排版修复或多阶段状态机改动，应在 PR 描述中提供便于直接核对的说明。若图片能明显展示行为差异，加入同条件的修复前后图片；若流程图能明显帮助理解控制流，加入一张简洁流程图。图片说明结果，流程图说明机制，测试清单说明覆盖范围，三者不能互相代替。
