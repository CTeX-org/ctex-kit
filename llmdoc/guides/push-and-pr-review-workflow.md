...
正式的本地代码审查必须由上下文隔离的子代理（sub-agent）完成。主代理可以检查自己的实现、整理验证证据和核实审查意见，但这些工作属于自检，不能代替正式审查。发起审查时固定 base 与 head，明确要求审查该提交范围内的代码、测试、文档和元数据；若审查后代码又有变化，必须由隔离的子代理对新增范围做相应的增量审查，主代理不能自行补签审查结论。
