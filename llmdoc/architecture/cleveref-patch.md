...
`cleveref02`／`cleveref03` 系列 `.tlg` 若出现 `[appendix][1][2147483647]A.` → `[chapter][1][]A.` 方向的基线 diff，与本文记录的 #725 根因链字面同构，属既有 Open 项（`hyperref#361`、`latex2e#2049` 明确不修）在新上游环境下的再暴露，不是新根因。#1048/#1050 排查上游 l3backend/pgf 漂移时遇到过这 4 个 `.tlg` diff，第一版曾误判为「根因未定位」——直接查本节即可确认根因链早已记全。在改这类基线前应先确认触发条件为何变化（例如具体是哪个上游包的哪次更新让 firstaid 的缺陷重新表现出来），否则等于把已知缺陷的新表现固化成预期值。
