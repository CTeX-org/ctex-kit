...
`box` 策略主要依赖 interchar transition 报告首尾类别，但 math 内容与 `\vrule` 都不触发该转换，capture 本身仍观察不到它们的类别。PR #1001（#998）为此在 box/wrapped-box 结束路径新增第二层判定：`\@@_boundary_if_capture_box_visible:` 在“无观察类别”前提下联合读盒尺寸（宽度非零、且高度或深度非零）与尾节点墨迹（`\hbox_unpack:` 复制到临时盒后取 `\tex_lastnodetype:D`，命中 char(0)/rule(3)/math(10)/kern(12) 才算“本层现场排版留下的痕迹”），命中即按 Default 首尾重建边界，使 `\mbox{$x$}`、`\mbox{\vrule...}` 与直接输入西文等价。透明保持集覆盖三类形状：零尺寸盒（`\null`、空 `\mbox`）、纯空白占位盒（空 `\makebox`、strut、零高 rule，包括 ctex 内核 `\[` display leader 用到的占位盒），以及尾节点是预排盒或填充 glue（hlist(1)/glue(11)）的命令（如 thuthesis `\thu@pad` 把已构造盒塞进末尾）——这类盒 capture 无法判断其内部真实类别，仍完整透明恢复。推断出的 default 经 `\@@_boundary_capture_report_first:n` 只补外层尚未取得的首类别，不直接覆写任何已观察末类别；内层结束时重放的 marker 留在父盒实际列表，父盒结束时由 `\@@_boundary_box_set_last_from_node:` 复制解包自身盒并读取可信尾 marker，校正本层 `last_tl`。因此 `\mbox{中\mbox{$x$}}`、对应 rule 形状及更深嵌套能逐层得到 Default 末类别，同时不会让 `\fcolorbox` 内部构造盒的推断越级降级外层已观察到的 CJK。`\mbox{\vrule...}` 的契约 oracle 仍是 Default 字母——“直接输入裸 rule/math 本身不触发 interchar”这条引擎机制边界不因本次修复改变；`\fbox{$x$}`/`\colorbox{yellow}{$x$}` 的尾节点是 frame/wrapper hlist，仍保持透明恢复，留作 #992 已知项。详见 [[../memory/decisions/992-command-boundary-capture-register]]「机制边界」。
...
这套校验现已在 Boundary→Default 与 Boundary→CJK 两个方向对称落地（#996，PR #1001）：`\@@_check_for_glue_skip:`（Boundary→CJK）升级为与 Default 方向同款的 `\@@_skip_if_interword:N` 谓词。`\g_@@_glue_check_pending_bool` 的跨盒生命周期也受到约束——新增 `\@@_glue_check_expire_stale:` 在顶层恢复链（无活跃 capture）探测到空列表时结束过期 pending，阻止其跨 `\hbox`/`\setbox` 分组存活；capture 活跃时不做该判断，以保护 ulem 等 stream 在内部字盒中合法携带的 pending。同构 glue 歧义窗口在两个方向依旧存在，workaround 不变（`\kern0pt`）。决策与复现证据见 [[../memory/decisions/992-command-boundary-capture-register]]「机制边界」。
...
| `siunitx` | `\unit`/`\qty`/`\num` 与 v2 旧名 `\si`/`\SI`（存在性守卫）注册为固定 Default 首尾的 `stream`；`\ang` oracle 语义未定暂不注册（#1000/#992） |
