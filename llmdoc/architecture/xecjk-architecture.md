...
`box` 策略依赖 interchar transition 报告首尾类别；math 内容与 `\vrule` 都不触发该转换，因而对 capture 不可见（#998）。`\mbox{$x$}`、`\mbox{\vrule...}` 会被判定为“无可见输出”并完整恢复入口状态，与命令实际留下可见排版内容矛盾，是与上文“显式同构 glue”并列的第二条已知机制边界；候选修复方向、workaround 与分支矩阵证据见 [[../memory/decisions/992-command-boundary-capture-register]]「机制边界」。
...
这套校验目前只在 Boundary→Default 方向落地；对称方向 Boundary→CJK（`\@@_check_for_glue_skip:`）尚无同款 finite/shrink/自然宽度校验，且 `\g_@@_glue_check_pending_bool` 会跨 `\setbox` 存活，导致跨盒的显式同构 glue 被误当作源码词间空格替换为 `CJKglue`（#996，预存缺陷，未随 #999 修复；workaround 同样是 `\kern0pt`）。决策与复现证据见 [[../memory/decisions/992-command-boundary-capture-register]]「机制边界」。
