...
#491 的历史回归通常只覆盖一个命令的一个单元，不能推出整类完成。#992 取代 #491 作为长期状态入口。PR #999 的原型矩阵在固定提交上全绿，但当时只覆盖 `xCJKecglue=false`。2026-07-21 补测 `true` 后发现嵌套盒子、中西混合内容和 `\null` 边界仍有差异，形成 #1003；PR #1005 通过恢复外层 `spacefactor` 和 post-transparent 的有界节点后缀修复这些普通命令单元。#992 的活表只记录已合并实现的状态：合并前可在 PR 上保存拟更新预览，不能提前把修复结果写回 issue 活表。
