...
若命令参数以“公式＋源码空格”结束，框架使用 `math-space` marker 表示参数内部已经保留一枚空格。它不把空格移到命令外，因此盒子尺寸、框线和 ulem 装饰范围不变；右侧为 CJK 且 `xCJKecglue=true` 时只补 `CJKecglue - 外层普通词间空格`，其余方向不再增加整份间距。紧接命令的另一枚源码空格按连续空格处理，显式 glue 仍由既有词间空格形状检查保护。
