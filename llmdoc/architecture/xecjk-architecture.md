...
推断出的 default 通过 `\@@_boundary_capture_report_first:n` 只补上尚未取得的首类别，不直接改写外层已经观察到的末类别。嵌套命令结束时写入的 marker 会留在外层盒子的节点列表末尾；外层盒子结束时，`\@@_boundary_box_set_last_from_node:` 复制并解包自身盒子，读取这个 marker，再更新本层 `last_tl`。这样，`\mbox{中\mbox{$x$}}` 及对应的 rule 情况可以逐层得到正确的 Default 末类别，同时不会让 `\fcolorbox` 命令内部的辅助盒子覆盖已经观察到的 CJK 末类别。`\mbox{\vrule...}` 的比较对象仍是 Default 字母，因为直接输入裸 rule/math 本身不触发 interchar 转换；`\fbox{$x$}` 和 `\colorbox{yellow}{$x$}` 的末节点是 frame/wrapper hlist，目前仍恢复命令前状态，记在 #992 的未处理项目中。详见 [[../memory/decisions/992-command-boundary-capture-register]]「机制边界」。
