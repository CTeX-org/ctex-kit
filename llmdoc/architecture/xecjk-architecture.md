...
对行内公式还要保留 `xCJKecglue` 的既有语义：`false` 让源码空格保持普通词间空格，`true` 才把它换成 `CJKecglue`。外层字体、颜色或盒子命令不能改变这项选择。`$x$` 是优先支持的输入形式；`\(x\)` 和 `\ensuremath{x}` 的目标行为与它相同，但可以按实现风险分阶段完成。测试必须分别检查左右边界，避免一侧多出的间距与另一侧缺少的间距在总宽度中互相抵消。完整决策和当前已知差异见 [[../memory/decisions/1002-inline-math-boundary-oracle]]。
...
结束路径根据入口前类别、实际首类别和左侧是否有源码空格重建左边界，排回盒子或保留原节点流，再把实际末类别写成 marker，让正常 Boundary→CJK/Default 恢复过程决定右边界。未观察到可见字符时，入口 marker 与源码空格原样恢复。
...
| `post-transparent` | 只能使用 after hook；仅在末尾盒子的宽、高、深均为零且确实遮住 marker 时，把原状态移到盒子后面 | 一般 `\null` |
...
`box` 策略通常依靠 interchar transition 报告首尾类别，但 math 内容与 `\vrule` 都不触发这种转换，所以 capture 看不到它们的类别。PR #1001（#998）在 box/wrapped-box 结束时增加了一项检查：如果没有观察到类别，`\@@_boundary_if_capture_box_visible:` 会同时检查盒子尺寸和末节点类型。盒子必须宽度非零、高度或深度非零，并且末节点是 char(0)、rule(3)、math(10) 或 kern(12)，才算直接排出了可见内容；此时把首尾报告为 Default。这个类别推断不等于已经证明所有外围源码空格都与直接输入一致：`\mbox{$x$}` 仍须与直接公式 `$x$` 比较，#1002 记录了 `xCJKecglue=false` 下 `10`、`11` 的差异。
...
推断出的 default 通过 `\@@_boundary_capture_report_first:n` 只补上尚未取得的首类别，不直接改写外层已经观察到的末类别。嵌套命令结束时写入的 marker 会留在外层盒子的节点列表末尾；外层盒子结束时，`\@@_boundary_box_set_last_from_node:` 复制并解包自身盒子，读取这个 marker，再更新本层 `last_tl`。这样，`\mbox{中\mbox{$x$}}` 及对应的 rule 情况可以逐层得到 Default 末类别，同时不会让 `\fcolorbox` 命令内部的辅助盒子覆盖已经观察到的 CJK 末类别。`\mbox{\vrule...}` 仍按 Default 类别检查；公式命令则必须以直接公式为 oracle。`\fbox{$x$}`、`\colorbox{yellow}{$x$}` 等公式包装的范围和基准统一记在 #1002。详见 [[../memory/decisions/992-command-boundary-capture-register]]「机制边界」。
