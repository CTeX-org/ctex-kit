...
`box` 策略通常依靠 interchar transition 报告首尾类别，但公式与 `\vrule` 都不触发这种转换。rule 继续由 `\@@_boundary_if_capture_box_visible:` 根据盒子尺寸和末节点类型按 Default 处理；行内公式使用独立的 `math` 类别。命令参数适配器只检查正文两端的明确公式语法：开头公式在 math-on 前报告首类别，结尾公式把当前最内层 capture 的末类别标为 `math`。显式花括号组可以递归检查；若花括号前是未知控制序列、活跃字符或另一个组，则它可能是宏参数，不能推断为可见公式。capture 结束时让明确的正文末类别覆盖命令内部留下的旧 marker，不把任意内部 math 节点猜成可见公式。实现不使用全局 `\everymath`，也不展开任意用户宏。
