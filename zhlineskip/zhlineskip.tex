...
\captionsetup{format=hang,labelformat=zhlabel,labelsep=zhcolon,font=sf}
\usepackage{enumitem}
\setlist{
...
\date{xxxx/xx/xx\hskip\ccwd\relax v1.0e}
...
使用范例：本宏包恢复数学环境\CJKecglue\env{array} 的行距，通过
\verb|\RestoreMathEnvironmentLeading{array}|\CJKecglue 实现。
\end{description}
...
\subsubsection*{例：设置正文行距为字号的 1.6\nobreak\CJKecglue 倍}
...
\subsubsection*{例：按照 Microsoft Word 设置“1.62\nobreak\CJKecglue 倍行距”}
...
如果插入的西文是引用参考文献的段落，那么使用\CJKecglue\env{quote}
或\CJKecglue\env{quotation} 环境就比较合适。
此时，可以直接在引用环境内部使用\CJKecglue\cmd{\linespread}\marg{real}
命令，建议将 \meta{real}\CJKecglue 设置在正文行距倍数的
$0.7$\nobreak\CJKecglue 倍左右。例如，在载入\CJKecglue
\pkg{zhlineskip} 宏包后，正文行距为字号的 $1.5$\nobreak\CJKecglue 倍，那么
\verb|\linespread{1.05}|\CJKecglue 就比较合适（$1.5 \times 0.7 = 1.05$）。
\begin{verbatim}
    \documentclass{ctexart}
    \usepackage{zhlineskip}
    \begin{document}
