...
- `issue1068-mwe.tex` / `issue1068-before-after.png` — 用于截图的精简版与对比图。
  ...
  **关于 AFTER 图里第二行多出的首行缩进**：那是正常的，且是手册承诺的语义。手册对
  `\ccwd` 的定义是「汉字字宽是相邻两个汉字中心之间的距离，**包含字距在内**。因此修改
  字距会间接修改字宽」，而 `autoindent` 默认取 `2\ccwd`，所以字间距变大时首行缩进跟着
  变宽（21.07874pt → 41.07874pt）。

  最直接的判据是跨引擎跨版本对照：**XeTeX 在未修改的 v2.6.4 上**，同一份样例的
  `\parindent` 就已经是 41.07874pt。LuaTeX 先前停在 21.07874pt，正是因为字间距被错误
  重置、`\ccwd` 也就没变。所以这里是把 LuaTeX 与 upTeX 对齐到既有行为，不是改变它。
...
1. 手册对 `\ccwd` 的定义——「汉字字宽是相邻两个汉字中心之间的距离，**包含字距在内**。
   因此修改字距会间接修改字宽」；`autoindent` 默认取 `2\ccwd`。`\ziju` 一节也重申
   「这个命令会影响 `\ccwd` 的值」。
2. **XeTeX 在未修改的 v2.6.4 上**，同一份样例的 `\parindent` 就已经是 41.07874pt、
   `\ccwd` 20.53937pt——与本次修好后 LuaTeX 的读数一致。
3. `\ctexset{linestretch=\maxdimen}` 这条 ctex 既有的、走 `auxi` 分支的配置下读数
   相同（这一条只能说明内部自洽，前两条才是实证）。
