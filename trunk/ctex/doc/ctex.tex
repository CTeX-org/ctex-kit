...
\usepackage[c5size,fntef,CJKspace,hyperref,UTF8]{ctex}
...
\makeatletter
\def\parg#1{\mbox{$\langle$\mbox{\itshape #1\/}$\rangle$}}
\def\@smarg#1{{\ttfamily\string{}\parg{#1}{\ttfamily\string}}}
\def\@marg#1{{\ttfamily\string{}{\rmfamily #1}{\ttfamily\string}}}
\def\marg{\@ifstar\@smarg\@marg}
\def\@soarg#1{{\ttfamily[}\parg{#1}{\ttfamily]}}
\def\@oarg#1{{\ttfamily[}{\rmfamily #1}{\ttfamily]}}
...
\date{\small 版本号：v1.11 \hskip 2\ccwd 修改日期：2012/04/16}
\maketitle
...
\toprule
  类别 & \textnormal{文件} & 说明 \\ \midrule
  文档类 & ctexart.cls & \texttt{article} 文档类的中文化版本 \\
         & ctexrep.cls & \texttt{report} 文档类的中文化版本 \\
         & ctexbook.cls & \texttt{book} 文档类的中文化版本 \\ \midrule
  格式   & ctexcap.sty & 提供全部功能，但\textbf{默认不修改缺省字体大小}，
                         需要使用 |cs4size| 或 |c5size| 选项来修改，适用于
                         由标准文档类衍生的其他文档类\\
         & ctex.sty    & 提供全部功能，但\textbf{默认不开启章节标题设置功能}，
                         需要使用 |caption| 选项来开启，也\textbf{默认不修改缺省字体大小}，
                         需要使用 |cs4size| 或 |c5size| 选项来修改，适用于非标准文档类 \\
\bottomrule
\end{tabularx}
\caption[\ctex{} 宏包的组成]{\ctex{} 宏包的组成。各个文档类和格
  式都有一个后缀为 \texttt{utf8} 的版本，相当于加了 \texttt{UTF8} 选项，保留
  它们是出于兼容性的考虑，不建议使用。}
\end{table}

\vskip 10pt
\emph{编译本说明文档需要先安装 \ctex{} 宏包。}
...
此外，对于 CJK 模式，\ctex{} 宏包会在 |\begin{document}| 和 |\end{document}|
...
注：CCT 方式\emph{不再建议}使用，目前仅作为向后兼容保留。

\ctex{} 宏包也可以配合新版的 CCT 使用，只需在使用 \ctex{} 宏包时加上 CCT 选项即可。
缺省 CCT 会使用 CJK 字库，因为这种字库方式比传统 CCT 字库更方便，兼容性也更好。
如果要使用传统 CCT 字库，则还要加上 CCTfont 选项。使用 CJK 字库时，只需要 CCT\LaTeX{}
提供的相关宏包，不需要使用 CCT 的编译程序。
...
\subsubsection{缺省字体大小选项} \label{class-opts}

\texttt{ctex.sty} 和 \texttt{ctexcap.sty} 默认不改变缺省字体大小，但下列选项也是有效的。
\begin{description}
\item[cs4size] 使用小四字号为缺省字体大小。
\item[c5size] 使用五号字为缺省字体大小。\emph{这个是 \ctex{} 文档类的缺省模式}
\end{description}

使用上述选项之一后，\LaTeXe{} 字体大小设置命令和实际大小对应关系见表 \ref{fontsize}。
...
字体命令         & 字号 & 磅数     && 字号 & 磅数     \\ \midrule
|\tiny|         & 七号 & |5.5bp|  && 小六 & |6.5bp|  \\
|\scriptsize|   & 小六 & |6.5bp|  && 六号 & |7.5bp|  \\
|\footnotesize| & 六号 & |7.5bp|  && 小五 & |9bp|    \\
|\small|        & 小五 & |9bp|    && 五号 & |10.5bp| \\
|\normalsize|   & 五号 & |10.5bp| && 小四 & |12bp|   \\
|\large|        & 小四 & |12bp|   && 小三 & |15bp|   \\
|\Large|        & 小三 & |15bp|   && 小二 & |18bp|   \\
|\LARGE|        & 小二 & |18bp|   && 二号 & |22bp|   \\
|\huge|         & 二号 & |22bp|   && 小一 & |24bp|   \\
|\Huge|         & 一号 & |26bp|   && 一号 & |26bp|   \\ \bottomrule
\end{tabular}
\caption{字体的各种大小}\label{fontsize}
\end{table}

你也可以通过 \ref{zihao} 一节介绍的命令 |\zihao|\marg*{字号} 来切换字体大小。

\subsubsection{CJK 模式中文编码选项}

下面的选项用于 CJK 模式编译选择 ctex 宏包的内部编码。
...
\emph{注意使用 \XeTeX{} 引擎的情况下总是内部使用 UTF-8 编码，所以不必使用
...
提供的中文字库在 \texttt{ctexopts.cfg} 文件中修改这些选项之一为缺省设置。
\begin{description}
\item[nofonts] 没有中文字库，此时没有中文字体命令可用。

\item[winfonts] 使用 Windows 的字体设置，默认为六种中易字体：宋体、仿宋、黑体、
楷体、隶书、幼圆（在使用 \XeTeX{} 时只有前四种）。该选项的结果将和老版本
\ctex{} 宏包完全一致。这是默认设置。\ctexdefault

\item[adobefonts] 在 xeCJK 模式中和使用 |DVIPDFMx| 作为输出驱动的 |CJK| 模式中
使用 Adobe 的四套字体：宋体、仿宋、黑体、楷体。其它方式无效。

\item[macfonts] 在 xeCJK 模式中使用 Mac OS X 定义的字体：宋体、细黑体和黑体、仿宋、楷体。
CJK 模式无效。

\item[fontset=none] 与 |nofonts| 选项等价。

\item[fontset=windows] 与 |winfonts| 选项等价。

\item[fontset=adobe] 与 |adobefonts| 选项等价。

\item[fontset=mac] 与 |macfonts| 选项等价。

\item[fontset=\parg{name}] 这里 \parg{name} 为你自定义的名字。使用这个选项后，\ctex{} 宏包将
载入名为 |ctex-fontset-|\parg{name}|.def| 的文件作为字体配置文件。因此，请先保证文件的存在。你
可以在当前工作目录或者本地 |TDS| 目录树下合适位置建立一个名为 |ctex-fontset-|\parg{name}|.def| 的
文件，用于个性化字体设置。然后通过使用 |fontset=|\parg{name} 选项来调用它。字体配置文件的具体写法
可以参考 \ctex{} 宏包 |fontset| 目录下的字体配置文件，例如 |ctex-fontset-windows.def|。
...
\item[zhmCJK] 载入 |zhmCJK| 宏包，它基于 |CJK| 宏包和 |zhmetrics| 机
制 ，可以动态地设置 CJK 字体，仅指定字体文件名，就完成原来十分复杂的 CJK 字体安装设置工作。

\item[CCT] 使用 CCT 做为底层的中文支持系统。
...
\begin{description}
\item[caption] 对于 |ctex.sty|，默认不开启章节标题设置功能，可以使用本选项
开启这个功能。\textbf{本选项只对 |ctex.sty| 有效。}

\item[sub3section] 将 |\paragraph| 命令产生的标题改为 section 类格式。
此时 |\subparagraph| 命令产生的标题会具有原来 |\paragraph| 的格式。
\emph{对于 |ctex.sty|，只有在使用 |caption| 选项后本选项才有效。}

\item[sub4section] 将 |\paragraph| 和 |\subparagraph| 命令产生的标题
都改为 section 类格式。\emph{对于 |ctex.sty|，只有在使用 |caption| 选项后本选项才有效。}

\item[cap] 使用中文的标题样式，缺省格式由 \texttt{ctexcap.cfg} 配置文件
内的定义给出。对于 \texttt{ctex.sty}，若没有使用 |caption| 选项，则该选项只影响
交叉引用中的数字和日期格式。\ctexdefault
...
\item[CJKspace] 载入 |CJKspace| 宏包，仅删除汉字之间的空格而保留中英文间的空格。
该选项只对 CJK 模式有效。
...
\subsubsection{字号、字距、字宽和缩进}\label{zihao}
...
使用 CJKnumb 提供的 |\CJKnumber| 和 |\CTEXdigits| 命令可以将阿拉伯数字转换为中文数字。
由于 \LaTeX{} 臭名昭著的脆弱命令的原因，当 |\CJKnumber| 等被用在章节标题等地方的时候，要
么出现错误无法使用，要么无法达到预期目的，例如在产生 PDF 书签的时候。于是我们编写
了 zhnumber 宏包，提供了可以适当展开的转换命令。

\DescribeMacro{\zhnumber}代替 CJKnumb 提供的 |\CJKnumber|。该命令的格式为
\begin{quote}
|\zhnumber|\marg*{number}
\end{quote}

\DescribeMacro{\zhdigits}|\zhdigits| 和 |\zhnumber| 命令类似，用于代替 CJKnumb 提供的
 |\CJKdigits| 命令。它和 |\zhnumber| 命令的不同之处在于转换后结果是中文数字串，而不
 是按照中文习惯的数字。该命令的格式为
\begin{quote}
|\zhdigits|\marg*{number}
\end{quote}

出于与旧版本兼容性的考虑，我们保留了 |\CTEXnumber| 和 |\CTEXdigits|。

\DescribeMacro{\CTEXnumber}用于将产生的中文数字保存下来。该命令的格式为
\begin{quote}
...
\DescribeMacro{\CTEXdigits}用于将产生的中文数字串保存下来。该命令的格式为
\begin{quote}
...
|\CTEXoptions| 多一个参数，用于指定设置对象。它对于 \ctex{} 文档类
和 |ctexcap.sty| 总是有效的，但\textbf{对于 |ctex.sty|，只有使用 |caption| 选项后才有效。}
基本格式是
\begin{quote}
...
\item[listfigurename] 插图目录
\item[listtablename] 表格目录
\item[figurename] 图
\item[tablename] 表
...
当前目录下，然后修改。\TeX{} 会优先使用当前目录下的同名文件。这样你可以针对不
同的应用设置不同的缺省配置文件。
...
\item[v1.11 2012/04/16]
处理与 |fontspec| v2.4 的兼容问题，一些 Bug 修复。

\item[v1.10 2012/04/16]
修改宏包结构，使 |ctex.sty| 就可以提供全部功能，将 |ctexcap.sty| 归入 |back| 目录。使
用 |kvoptions| 宏包重构宏包选项，通过 |fontset=|\parg{name} 的宏包选项接口支持本地
字体集的载入，增加对 |zhmCJK| 和 |CJKspace| 的支持。改用 |zhnumber| 宏包处理中文数字格式。

\item[v1.03 2012/01/07]
修正 CJK 方式下 |breqn| 宏包无法使用的 BUG。更新宏包文档。

	为 Mac OS X 用户在 |xeCJK| 模式下增加 |macfonts| 字体集选项。

\item[v1.02c 2011/03/11]
修改 |hyperref| 选项的行为，使 |hyperref| 宏包可以在用户指定的位
...
修正 pdf\LaTeX{} 与 \XeLaTeX{} 中生成 PDF 书签的问题。
