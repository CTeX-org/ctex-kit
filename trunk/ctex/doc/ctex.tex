...
\date{\small 版本号：v1.03 \hskip 2\ccwd 修改日期：2012/01/07}
\maketitle
...
\begin{tabular}{l>{\ttfamily}lp{.5\textwidth}} \hline
  ...
  格式   & ctexcap.sty & 提供中文支持与汉化标题设置，用于由标准文档类衍生的其
                         他文档类\\
         & ctex.sty    & 仅提供基本中文支持，用于非标准文档类 \\ \hline
\end{tabular}
\caption[\ctex{} 宏包的组成]{\ctex{} 宏包的组成。其中 \ctex{} 文档类除了提供
  \texttt{ctexcap} 格式的全部功能，还进行了额外的默认字号设置。各个文档类和格
  式都有一个后缀为 \texttt{utf8} 的版本，相当于加了 \texttt{UTF8} 选项。}
\end{table}
...
在 CJK 模式（即不使用 \XeTeX{} 时）下，该选项将使用 winfonts 选项的设置。
