...
# 从 dtx 中抽取指定版本或全部 \changes 条目, 返回版本号和文本的元组列表
def extract(dtx_path: str, target_ver: str) -> list[tuple[str, str]]:
    ...
    with open(dtx_path, encoding="utf-8", errors="replace") as f:
        lines = f.readlines()
    # 如果传入的 target_ver 为 "all", 则匹配任意版本的 "\\changes{", 否则匹配特定版本
    tag = "\\changes{" \
        if target_ver.lower() == "all" else "\\changes{" + target_ver + "}"
    # 修改存储结构为元组列表以记录对应的版本号
    ...
    while i < len(lines):
        ...
        text = re.sub(fr"([{CJK_RANGE}])\s+(?=[{CJK_RANGE}])", r"\1", text)
        # 处理新条目前，清空重用缓存列表
        math_blocks.clear()
        verbatim_blocks.clear()
        ...
        if text:
            # 将清洗后的文本连同其版本号一起存入
            entries.append((current_ver, text))
    return entries
...
def main() -> int:
    ...
    package_name = os.path.commonprefix(basenames).rstrip("-_") \
        if basenames else "package"
    if not package_name and basenames:
        # 防止切片后为空的极端防御退路
        package_name = basenames[0]
    # 引入字典按版本号对变更条目进行归类分组
    version_groups: dict[str, list[str]] = {}
    for file in dtx_files:
        if os.path.isfile(file):
            entries = extract(file, target_ver)
            for ver, e in entries:
                if (ver, e) not in global_seen:
                    global_seen.add((ver, e))
                    # 将变更内容追加到对应的版本列表中
                    version_groups.setdefault(ver, []).append(e)
    # 定义语义化版本（Semantic Versioning）排序键函数,
    # 将数字部分转为 int: ✓ 确保 [x.]+ 10 > [x.]+ 1
    #                     ✗ 阻止 [x.]+ 10 = [x.]+ 1
