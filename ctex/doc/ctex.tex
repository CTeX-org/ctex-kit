...
\usepackage[UTF8,fntef,adobefonts]{ctex}
\usepackage{texnames}
...
中文环境，可以配合大多数文档类使用。而~\texttt{ctexcap.sty}~则是在~\texttt{ctex.sty}~
的基础上对~\LaTeX{}~的三个标准文档类的格式进行修改以符合中文习惯，
该宏包只能配合这三个标准文档类使用。
...
\vskip 10pt
{\kaishu
这份说明文档可以通过用~\LaTeX{}~编译~\texttt{ctex.tex}~文件来得到。
编译说明文档需要先安装~\ctex{}~宏包。}
...
{\kaishu 除了~\ref{class-opts}~和~\ref{caption-opts}~的选项，
...
\item[c5size] 使用五号字为缺省字体大小。{\kaishu 这个是
~\ctex{}~文档类的缺省模式。}
\end{description}
...
下面的选项用于选择中文的编码。
\begin{description}
\item[GBK] 使用~GBK~编码。{\kaishu 这个是~\ctex{}~宏包的缺省模式。}

\item[UTF8] 使用~UTF8~编码。
\end{description}
...
\item[cjkfonts] 在~CJK~模式（即不使用~Xe\TeX{}~时）使用~CJKfonts~中的两套
文鼎字库：宋体和楷体。在~xeCJK~模式（即使用~Xe\TeX{}~时），该选项将使用
~adobefonts~选项的设置。

\item[winfonts] 使用~Windows~的字体设置，默认为六种中易字体：宋体、仿宋、黑体、
楷体、隶书、幼圆。该选项的结果将和老版本~\ctex{}~宏包完全一致。
...
{\kaishu 这个是~\ctex{}~宏包的缺省模式。}

\item[nocap] 保留使用英文的标题样式。

\item[punct] 对中文标点的位置（宽度）进行调整。{\kaishu 这个是~\ctex{}~宏包的缺省模式。}
...
总结一下：\ctex{}~宏包的缺省选项是~GBK nospace cap punct indent，\ctex{}~文档类
的缺省选项是~GBK nospace cap punct indent c5size。
...
{\kaishu 在~v0.7~版本之前，如果以上命令的参数中包含中文字符，则命令必须放在
...
\item 吴凌云~(aloft@ctex.org)
\item 江疆~(gzjjgod@gmail.com)
\item 王越~(yuleopen@gmail.com)
\item 刘海洋~(LeoLiu.PKU@gmail.com)
