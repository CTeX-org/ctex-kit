...
\date{\small 版本号：v0.91 \hskip 2\ccwd 修改日期：2009/05/23}
\maketitle
...
{\heiti 除了~\ref{class-opts}~和~\ref{caption-opts}~的选项，
...
下面的选项用于选择中文的编码。
\begin{description}
\item[GBK] 使用~GBK~编码。{\heiti 这个是~\ctex{}~宏包的缺省模式。}
...
内的定义给出。{\heiti 这个是~\ctex{}~宏包的缺省模式。}

\item[nocap] 保留使用英文的标题样式。

\item[punct] 对中文标点的位置（宽度）进行调整。{\heiti 这个是~\ctex{}~宏包的缺省模式。}
