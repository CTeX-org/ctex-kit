...
#366 的文档审计报告在 PR #1062 合并后才送到，报出的四条都已在本分支补修，没有遗留：

- 手册那句「实测 `\zhrod{1{ab}4}` 报八条错误」，条数在任何口径下都复现不出来。实测
  TeX 错误恒为四条，缺字警告随字体与引擎而变（0／2／3／9 都出现过），合计得 4／6／7／13，
  没有 8。已改成写明四条错误的具体内容、并说明缺字警告条数不固定，不再写死总数。
- driver 区两行新增中文注释用了半角逗号与句号，已改全角（`font=` 作为代码片段保留半角）。
- `\changes` 与 `CHANGELOG.md` 漏记公开命令 `\zhrodsetup`，已补。
- `build.lua` 里「仅 `rod-engine01` 需要 `.uptex.tlg`」写于 `rod-engine02` 从它拆出之前，
  已更正为两个用例各有一份。

从这几条里得到的一条经验，值得后来人留意：**手册里凡写具体数字，都要写明计数口径。**
上面那个「八条」之所以站不住，正是因为它把「随字体与引擎变化的缺字警告」和「固定的 TeX
错误」混在一个数里数，于是换个上下文就对不上。要么只写口径稳定的那部分，要么把口径一并写出。
